    ...
    <div style="max-width:600px; margin:0 auto; background-color:#fff; border:1px solid #ddd; border-radius:8px; overflow:hidden;">
        ...
        <div style="padding:10px 20px;">
            ...
            <div style="margin:10px 0 20px 0;">
                @if (($slip->installment_date_due ?? false) && (dataDiferencaDias($slip->installment_date_due) < 0))
                ...
                <table style="margin-bottom:10px; box-shadow: 0px 4px 10px rgba(0, 0, 0, 0.2);width: 100% !important; border-collapse:collapse; margin-bottom:20px; border:1px solid #ddd; border-radius:8px; overflow:hidden;">
                    <tr>
                        <th colspan="2" style="border-top:1px solid rgb(177, 177, 177); padding:10px; font-weight:bold; background-color:#07ace6; text-align:left; color:#fff;text-transform:uppercase;">
                            ...
                            <span>{{ __($slip->installment_pay_type ?? null) }}</span>
                        </th>
                    </tr>
                    ...
                    <tr style="background-color:#F5F5F5;">
                        ...
                        <td style="background-color:#F5F5F5; border-top:1px solid rgb(177, 177, 177); padding:10px; text-align:right;">
                            <div style="font-weight:bold; color:#333;">{{ mb_strtoupper($slip->order->order_control) }}</div>
                        </td>
                    </tr>
                    <tr style="background-color:#F5F5F5;">
                        ...
                        <td style="background-color:#F5F5F5; border-top:1px solid rgb(177, 177, 177); padding:10px; text-align:right;">
                            <div style="font-weight:bold; color:#333;">{{ mb_strtoupper(__($slip->installment_description ?? '---')) }}</div>
                        </td>
                    </tr>
                    <tr style="background-color:#F5F5F5;">
                        ...
                        <td style="background-color:#F5F5F5; border-top:1px solid rgb(177, 177, 177); padding:10px; text-align:right;">
                            <div style="font-weight:bold; color:#333;">{{ dataData($slip->installment_date_due,ago:true) }}</div>
                        </td>
                    </tr>
                    <tr style="background-color:#F5F5F5;">
                        ...
                        <td style="background-color:#F5F5F5; border-top:1px solid rgb(177, 177, 177); padding:10px; text-align:right;">
                            <div style="font-weight:bold; color:#333;">{{ toMoney($slip->installment_value,'R$ ') }}</div>
                        </td>
                    </tr>
                    <tr style="background-color:#F5F5F5;">
                        <td style="background-color:#F5F5F5; border-top:1px solid rgb(177, 177, 177); padding:10px; font-weight:bold; color:#07ace6; text-align:left;">DESCRIÇÃO</td>
                        <td style="background-color:#F5F5F5; border-top:1px solid rgb(177, 177, 177); padding:10px; text-align:right;">
                            <div style="font-weight:bold; color:#333;">{{ $slip->order->slip_description ?? '---' }}</div>
                        </td>
                    </tr>
                </table>
            @else
            ...
            <div style="text-align:center; margin:20px 0;">
                <a href="{{ route('compra-exibir', ['localizador' => $slip->order->order_control]) }}"
                    target="_blank"
            ...
            <div style="text-align:center; font-size:14px; margin-top:20px; border:1px solid #ddd; border-radius:8px; padding:10px;">
                @if (($slip->order->event->organizer ?? false) && $slip->order->event->organizer->owner_phone_ddd && $slip->order->event->organizer->owner_phone_num)
                    <p>Dúvidas? Fale com o organizador pelo Telefone / WhatsApp <strong>{{ putMask($slip->order->event->organizer->owner_phone_ddd.$slip->order->event->organizer->owner_phone_num,'telefone') }}</strong></p>
                @else
                    <p>Dúvidas? Fale com a empresa pelo Telefone / WhatsApp <strong>{{ putMask($slip->order->event->customer->comercial_contact_ddd.$slip->order->event->customer->comercial_contact_num,'telefone') }}</strong></p>
                @endif
            </div>
