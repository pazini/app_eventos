...
<?php endif; ?>
    ...
    <?php if($campaignList->count()): ?>
        ...
        <main class="max-w-7xl mx-auto px-6 md:px-8 pt-6 pb-12">
            ...
            <?php if($campaignList->take(3)->count() > 0): ?>
                <section class="mb-16">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                        <?php $__currentLoopData = $campaignList->take(3); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $campaign): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <?php
                            ...
                            <a href="<?php echo e(campanhaUrl($campaign->customer_organization_slug, $campaign->slug)); ?>" class="group block">
            ...
            <?php if($campaignList->skip(3)->count() > 0): ?>
                <section id="todas">
                    <div class="flex items-center justify-between mb-6">
                        <h2 class="text-2xl font-bold text-gray-900">Outras Campanhas</h2>
                        <span class="text-sm text-gray-500"><?php echo e($campaignList->count()); ?> campanhas</span>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                        <?php $__currentLoopData = $campaignList->skip(3); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $campaign): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <?php
                                // Função helper para gerar URL completa da imagem
                                $getImageUrl = function($url) {
                                    if (empty($url)) return null;
                            ...
                            <a href="<?php echo e(campanhaUrl($campaign->customer_organization_slug, $campaign->slug)); ?>" class="group block">
                                <div class="bg-white rounded-xl overflow-hidden border border-gray-200 hover:shadow-lg transition-all duration-200 hover:-translate-y-0.5">
                                    ...
                                    <div class="relative h-64 bg-gray-100 overflow-hidden <?php echo e($useLogoStyle ? 'flex items-center justify-center' : ''); ?>">
                                        <img
                                            src="<?php echo e($urlImage); ?>"
                                            alt="<?php echo e($campaign->name); ?>"
                                            class="<?php echo e($useLogoStyle ? 'max-w-[70%] max-h-[70%] w-auto h-auto object-contain' : 'w-full h-full object-cover'); ?> transition-transform duration-500 group-hover:scale-105"
                                            onerror="this.onerror=null; this.src='<?php echo e(appLogo(true)); ?>'; this.className='max-w-[70%] max-h-[70%] w-auto h-auto object-contain transition-transform duration-500 group-hover:scale-105';"
                                        />
                                        <?php if($campaign->datetime_finish && $campaign->datetime_finish < now()): ?>
                                    ...
                                    <div class="p-4 md:p-5">
                                        ...
                                        <div class="space-y-1 md:space-y-1.5">
                                            <?php if($campaign->organizer && $campaign->organizer->organizer_name_full): ?>
                                                <div class="text-xs md:text-sm text-gray-600">
                                                    <span class="capitalize font-medium"><?php echo e(Str::limit($campaign->organizer->organizer_name_full, 100)); ?></span>
                                                </div>
                                            <?php elseif($campaign->organizer && $campaign->organizer->organizer_name): ?>
                                                <div class="text-xs md:text-sm text-gray-600">
                                                    <span class="capitalize font-medium"><?php echo e(Str::limit($campaign->organizer->organizer_name, 100)); ?></span>
    ...
    <?php if($userUuid ?? false): ?>
        <div class="fixed bottom-4 right-4 bg-white border border-gray-200 rounded-lg p-3 shadow-lg max-w-xs">
            <div class="flex items-center gap-2">
                <p class="text-xs text-gray-600 font-mono truncate flex-1" id="userUuid"><?php echo e($userUuid); ?></p>
                <button
                    onclick="copyToClipboard('<?php echo e($userUuid); ?>')"
...
<?php /**PATH /home/igrnc24/public_html_sistemas/campanhas-eventos-proeventpay-transicao/resources/views/livewire/app-campanha-home.blade.php ENDPATH**/ ?>