<div class="mb-10">
    ...
    {{-- ============================================
        MODAIS - FORA DOS IFS PARA FUNCIONAREM CORRETAMENTE
    ============================================= --}}

    {{-- MODAL NOVA FILIAL --}}
    <x-modal.card title="Nova Filial" blur wire:model="organizationCadastrar">
    ...
    <x-modal.card title="Nova Subdivisão" blur wire:model="organizationSubCadastrar">
        <div class="grid grid-cols-1 md:grid-cols-6 gap-4">
            <div class="col-span-full">
                <x-native-select label="Filial" wire:model.live="organization_id_for_subs" class="uppercase">
                ...
                @error('organization_id_for_subs')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>

            @if($organization_id_for_subs)
                <div class="col-span-full">
                    <p class="text-sm text-gray-600 mb-4">
                        Cadastre uma nova subdivisão para <strong class="text-gray-900">{{ $this->organizations->firstWhere('id', $organization_id_for_subs)->organization_name ?? $this->organization->organization_name ?? 'a filial selecionada' }}</strong>
                    </p>
                </div>
        ...
        <x-slot name="footer">
            <div class="flex justify-between gap-x-4">
                ...
                <div class="flex gap-2">
                    <x-button flat label="Cancelar" x-on:click="close" />
                    <x-button primary label="CADASTRAR" wire:click="cadastrarCentroCusto(true)" :disabled="!$organization_id_for_subs" />
                </div>
            </div>
        </x-slot>
    </x-modal.card>
    ...
    @if ($customer ?? false)
        ...
        <div class="{{ setClass('divContentHeader') }} relative overflow-hidden">
            ...
            <div class="relative z-10 w-full space-y-6">
                ...
                <nav class="grid grid-cols-1 md:grid-cols-4 gap-4" role="tablist">
                    ...
                    <button
                        wire:click="switchTab('setores')"
                        class="px-4 py-2 font-medium transition-all duration-200 rounded-lg shadow-sm
                            {{ ($activeTab ?? 'organizadores') === 'setores'
                                ? 'bg-white text-sky-600'
                                : 'bg-white/10 backdrop-blur-sm border border-white/30 text-white hover:bg-white/20' }}"
                        role="tab"
                        aria-selected="{{ ($activeTab ?? 'organizadores') === 'setores' ? 'true' : 'false' }}">
                        <div class="flex items-center gap-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @if (($activeTab ?? 'organizadores') === 'organizadores')
            ...
            {{-- MODAL REMOVE CENTRO DE CUSTO --}}
            <x-modal.card title="Remover Centro de Custo" blur wire:model.defer="removerOrganizer">
                @if ($this->organizer ?? false)
                    <div class="grid grid-cols-1 md:grid-cols-6">
                        ...
                        <div class="col-span-1">
                            ...
                            @else
                                {!! setLabel('eventos', $this->organizer->events->count() > 0 ? $this->organizer->events->count() : 'nenhum') !!}
                            @endif
                        </div>
                        ...
                        @if($context === 'campanhas')
                            ...
                            @endif
                        @else
                            @if ($this->organizer->events->count())
                                <div class="col-span-full mb-4">
                                    <hr>
                                </div>
                                @foreach ($this->organizer->events as $event)
                                    <div class="col-span-full">
                                        <div class="bg-gray-50 pt-6 px-4 shadow border-b">
                                            {!! setLabel(' ', $event->event_name) !!}
                                        </div>
                                    </div>
                                @endforeach
                            @endif
                    ...
                    <x-slot name="footer">
                        @if (is_null($this->organizer->organization_id))
                            {!! setLabel(' ', "<span class='text-red-600 font-bold'>O centro de custo da própria empresa não pode ser removido</span>") !!}
                        @elseif(($context === 'campanhas' && $this->organizer->campaigns->count()) || ($context !== 'campanhas' && $this->organizer->events->count()))
                            {!! setLabel(' ', "<span class='text-red-600 font-bold'>O centro de custo não pode ser removido</span>") !!}
                        @else
                            <div class="flex justify-between gap-x-4">
                                <div></div>
                                <div class="flex">
                                    <x-button flat label="Cancelar" wire:click="$set('removerOrganizer', false)" />
                                    <x-button negative label="Remover" onclick="confirm('Confirma remoção do centro de custo?') || event.stopImmediatePropagation()" wire:click="removerOrganizer('{{ $this->organizer->id }}',true)" />
                                </div>
                            </div>
                        @endif
                    </x-slot>
                @else
                    <div class="col-span-full flex justify-between items-center bg-white hover:bg-gray-50 py-2 px-4 shadow border-b">
                        <div class="font-light">Nenhum centro de custo encontrado</div>
                    </div>
                @endif
            </x-modal.card>
                ...
                @if (!($this->organizer ?? false))
                    <div class="space-y-4">
                        ...
                        @if ($this->organization_id ?? false)
                            @if (($this->organizationsSub ?? false) && $this->organizationsSub->count())
                                <x-native-select
                                    label="Selecione a Subdivisão"
                                    wire:key="organization_sub_id_{{ __LINE__ }}"
                                    wire:model.defer="organization_sub_id"
                                    placeholder="Escolha a subdivisão"
                                >
                                    <option value="">-- Selecione uma Subdivisão --</option>
                                    @foreach ($this->organizationsSub ?? [] as $item)
                                        <option value="{{ $item->id }}">
                                            {{ $item->organization_sub_name }} | {{ $item->organization_sub_description }}
                                        </option>
                                    @endforeach
                                </x-native-select>
                            @else
                                <div class="p-3 bg-yellow-50 border border-yellow-200 rounded-md mb-4">
                                    <p class="text-sm text-yellow-800">Não existem subdivisões disponíveis para esta filial.</p>
                                </div>
                            @endif
                        @else
                            <div class="p-3 bg-gray-50 border border-gray-200 rounded-md">
                                <p class="text-sm text-gray-700">Selecione uma filial para carregar as subdivisões.</p>
                            </div>
                        @endif
                    </div>
                @elseif (($this->organizer ?? false) && !is_null($this->organizer->organization_id))
                    <div class="space-y-3">
                        <div class="flex items-center">
                            <h3 class="text-lg font-medium text-gray-900">Filial/Setor Atual</h3>
                        </div>

                        <div class="p-3 mb-4 bg-gray-50 border border-gray-200 rounded-md">
                            <p class="text-sm text-gray-700">
                                A filial e a subdivisão não podem ser alteradas durante a edição.
                            </p>
                        </div>
                    </div>
                @elseif (($this->organizer ?? false) && is_null($this->organizer->organization_id))
                ...
                <div class="space-y-4 mt-4">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            <x-input
                                label="E-mail"
                                wire:key="owner_email_{{ __LINE__ }}"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <x-native-select
                                label="DDD"
                                wire:key="owner_phone_ddd_{{ __LINE__ }}"
            ...
            <div class="w-full max-w-7xl mx-auto bg-white border-x border-b px-16 py-4">
                <div class="flex flex-col md:flex-row gap-4 items-end">
                    ...
                    <div class="w-full md:w-64">
                        ...
                        <x-native-select xs wire:model.live="organization_id" class="text-gray-700 w-full uppercase pt-2 pb-2">
                    ...
                    <div class="flex-1">
                        ...
                        <x-input
                            wire:model.live.debounce.300ms="searchOrganizadores"
            ...
            <div class="w-full max-w-7xl mx-auto bg-white border-x border-b px-16 py-4">
                <div class="pt-6 space-y-6">
                    @forelse ($organizers ? $organizers->sortBy('organizer_name_full') : [] as $organizer)
                            ...
                            <div class="p-6 grid grid-cols-12 gap-4">
                                        ...
                                        <div class="flex-1 space-y-2">
                                            <div class="flex items-center gap-3 flex-wrap">
                                                ...
                                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                                    ...
                                                    @else
                                                        {{ $organizer->events->count() }} evento{{ $organizer->events->count() != 1 ? 's' : '' }}
                                                    @endif
                                                </span>
                                            </div>
                                        </div>
                                        <div class="flex items-center gap-2">
                                            ...
                                            @if(!is_null($organizer->organization_id))
                                                <x-button
                                                    ...
                                                    icon="trash"
                                                    label="Remover"
                                                    wire:click="removerOrganizer('{{ $organizer->id }}')"
                                                    class="w-auto"
                                                />
                                            @else
                                ...
                                <div class="col-span-12" >
                                    ...
                                    <div class="flex justify-between items-center mb-4">
                                        <div><span>Usuários Associados ({{ $organizer->users->whereNotIn('email',['admin@empresateste.com'])->count() }})</span></div>
            ...
            <div class="w-full max-w-7xl mx-auto bg-white border-x border-b px-16 py-4">
                <div class="flex flex-col md:flex-row gap-4 items-end">
                    ...
                    <div class="flex-1">
                        <label class="block text-sm font-medium text-gray-700 mb-2">Buscar</label>
                        <x-input
                            wire:model.live.debounce.300ms="searchUsuarios"
            ...
            <div class="{{ setClass('divContent') }} rounded-none border-x border-b">
                ...
                <div class="flex gap-4 items-center">
                    @if (!$organizationCadastrar ?? false)
                        <x-button flat lime label="NOVA FILIAL" wire:click="cadastrarFilial" class="px-6 py-2 font-medium hover:bg-lime-50 transition-colors duration-200 rounded-lg" />
                    @endif
                </div>
            </div>
                    ...
                    @forelse ($organizations ?? [] as $organizationItem)
                        <div class="bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden">
                            <div class="p-6 grid grid-cols-12 gap-4">
                                ...
                                @if (($editingOrganizationId ?? false) && $editingOrganizationId == $organizationItem->id)
            ...
            <div class="{{ setClass('divContent') }} rounded-none border-x border-b">
                ...
                <div class="flex flex-col sm:flex-row gap-4 items-stretch sm:items-center w-full sm:w-auto">
                    {{-- <div class="min-w-0 flex-1 sm:min-w-64">
                        <x-native-select xs wire:model.live="organization_id_for_subs" class="text-gray-700 w-full uppercase pt-2 pb-2">
                            <option value="">Selecione uma Filial</option>
                            @foreach (($this->organizations ?? false) ? $this->organizations->sortBy('organization_name') : [] as $organization)
                            <option value="{{ $organization->id }}" class="uppercase">{{ $organization->organization_name }}</option>
                            @endforeach
            ...
            <div class="w-full max-w-7xl mx-auto bg-white border-x border-b px-16 py-4">
                <div class="flex flex-col md:flex-row gap-4 items-end">
                    ...
                    <div class="flex-1">
                        <label class="block text-sm font-medium text-gray-700 mb-2">Buscar</label>
                        <x-input
                            wire:model.live.debounce.300ms="searchOrganizationSubs"
                            placeholder="Buscar centro de custo por nome, email ou descrição..."
                            icon="search"
                            class="w-full"
                        />
                            ...
                            <div class="bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden">
                                <div class="p-6 grid grid-cols-12 gap-4">
                                    ...
                                    @if (($organization_sub_id ?? false) && $organization_sub_id == $organizationSubItem->id)
                                        <div class="col-span-12">
                                            <div class="bg-blue-50 rounded-lg p-6 border border-blue-200">
                                                <h4 class="text-lg font-semibold text-gray-800 mb-4 uppercase">Editar Subdivisão</h4>
                            ...
                            <div class="text-center py-12">
                                <div class="max-w-md mx-auto">
                                    ...
                                    <p class="text-gray-600 mb-6">
                                        {{ ($organization_id_for_subs ?? false) ? 'Esta filial ainda não possui subdivisões cadastradas.' : 'Não há subdivisões cadastradas nas filiais.' }}
                                    </p>
