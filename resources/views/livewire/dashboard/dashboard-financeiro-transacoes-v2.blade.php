                    ...
                    @if ($pedidos ?? false)
                        ...
                        <div class="w-full bg-white my-4">
                            ...
                            <table id="example" class="display" style="width:100%">
                                <thead>
                                    <tr>
                                        @foreach (['DATA','SITUAÇÃO / LOCALIZADOR','COMPRADOR','QTD','VALOR','PAGAMENTOS'] as $column)
...
@endphp
<div class="{{ setClass($statusClass,'py-0.5 px-2 text-xs mb-1') }}" title="{{$pedido->status}}">{{ __($pedido->status) }}</div>
<span class="sr-only">:::</span>
<div class="text-gray-600 font-semibold">{{ $pedido->order_control }}</div>
</td>
{{-- COMPRADOR --}}
<td wire:click="verDetalhes('{{ $pedido->id }}')" class="text-gray-500 font-medium truncate">
<div class="uppercase truncate">{{ ucwords($pedido->buyer_name) }}</div>
<span class="sr-only">:::</span>
