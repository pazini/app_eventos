            ...
            <div class="relative z-10 p-4">
                <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-3">
                    <div class="flex-1">
                        <div class="flex items-center space-x-2">
                            ...
                            <div>
                                ...
                                <p class="text-white/90 text-xs mt-0.5">{{ $target->event_name }} - Consulta: {{ now()->format('d/m/Y H:i') }}</p>
                            </div>
                        </div>
                    </div>
                    <div class="flex items-center gap-2">
                        @auth
                            @if (isAdmin())
                                <x-button outline white xs label="FILTAR" right-icon="filter" class="hover:bg-white/20" />
                            @endif
                        @endauth
                        <x-button white xs label="LER QR CODE" right-icon="qrcode" href="{{ route('checkin-target',['ref_target' => 'event','ref_target_slug' => $target->event_slug]) }}" class="hover:bg-white/20" />
                        @auth
                            <a href="{{ route('evento-by-uuid', $target->id) }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-white bg-white/20 border border-white/40 rounded hover:bg-white/30 hover:border-white/60 transition-all">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3 mb-4">
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                    <div class="bg-gradient-to-br from-gray-800 to-gray-900 px-4 py-3 border-b border-gray-200">
                        ...
                        <div class="text-2xl font-bold text-white mt-1">{{ $ingressoQtdVendidos }}</div>
                    </div>
                </div>
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                    <div class="bg-gradient-to-br from-blue-500 to-blue-600 px-4 py-3 border-b border-gray-200">
                        ...
                        <div class="text-2xl font-bold text-white mt-1">{{ $contador_disponivel }}</div>
                    </div>
                </div>
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                    <div class="bg-gradient-to-br from-yellow-500 to-yellow-600 px-4 py-3 border-b border-gray-200">
                        ...
                        <div class="text-2xl font-bold text-white mt-1">{{ $contador_reserva_temp }}</div>
                    </div>
                </div>
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                    <div class="bg-gradient-to-br from-green-500 to-green-600 px-4 py-3 border-b border-gray-200">
                        ...
                        <div class="text-2xl font-bold text-white mt-1">{{ $contador_utilizado }}</div>
                    </div>
                </div>
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                    <div class="bg-gradient-to-br from-red-500 to-red-600 px-4 py-3 border-b border-gray-200">
                        ...
                        <div class="text-2xl font-bold text-white mt-1">{{ $contador_cancelado }}</div>
                    </div>
                </div>
            </div>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($participantes->sortBy('user_name') ?? [] as $participante)
                                    ...
                                    <tr class="hover:bg-gray-50 transition-colors">
                                        {{-- LOCALIZADOR / STATUS (COMBINADO) --}}
                                        <td class="px-3 py-2 text-xs whitespace-nowrap" data-order="{{ $statusOrder }}">
                                            <div class="flex flex-col gap-1">
                                                @if($href != '#')
                                                    <a href="{{ $href }}" class="text-blue-600 hover:text-blue-800 font-mono font-semibold hover:underline whitespace-nowrap">
                                                        {{ $participante->ticket_control }}
                                                    </a>
                                                @else
                                                    <span class="text-gray-400 font-mono font-semibold whitespace-nowrap">{{ $participante->ticket_control }}</span>
                                                @endif
                                                <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium {{ $statusBadgeClass }} whitespace-nowrap">
                                                    {{ mb_strtoupper(__($participante->ticket_status)) }}
                                                </span>
                                            </div>
                                        </td>
                                        ...
                                        <td class="px-3 py-2 text-xs text-gray-900 uppercase whitespace-nowrap">
                                            {{ mb_strtoupper($participante->user_name) }}
                                        </td>

                                        {{-- TELEFONE --}}
                                        <td class="px-3 py-2 text-xs text-gray-600 whitespace-nowrap">
                                            ({{ $participante->user_contact_ddd ?? '--' }}) {{ $participante->user_contact_num ?? '---- - ----' }}
                                        </td>
                                        ...
                                        {{-- TIPO --}}
                                        <td class="px-3 py-2 text-xs text-gray-700 uppercase whitespace-nowrap">
                                            {{ $participante->event_ticket_name }}
                                        </td>

                                        {{-- GERADO EM --}}
                                        <td class="px-3 py-2 text-xs text-gray-600 whitespace-nowrap">
                                            {{ $participante->created_at ? $participante->created_at->format('d/m/Y H:i') : '---' }}
                                        </td>

                                        {{-- COMPRADOR --}}
                                        <td class="px-3 py-2 text-xs text-gray-900 uppercase whitespace-nowrap">
                                            {{ $participante->order->buyer_name ?? '---' }}
                                        </td>

                                        {{-- EMAIL --}}
                                        <td class="px-3 py-2 text-xs text-gray-600 lowercase whitespace-nowrap">
                                            {{ mb_strtolower($participante->order->buyer_email ?? '---') }}
                                        </td>

                                        {{-- DOCUMENTO --}}
                                        <td class="px-3 py-2 text-xs text-gray-700 whitespace-nowrap">
                                            {{ $participante->order->buyer_doc_num ? putMask($participante->order->buyer_doc_num,$participante->order->buyer_doc_type) : '---' }}
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
