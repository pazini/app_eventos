                    ...
                    <div class="flex-1">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                ...
                                <p class="text-white/90 text-sm">{{ $target->event_name }} - {{ $order->order_control ?? '--' }}</p>
                            </div>
                        </div>
                    </div>
                    <div class="flex items-center gap-3">
                        <x-button flat white icon="x" label="FECHAR" wire:click="$set('cancelarOrder', false)" class="hover:bg-white/20" />
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div class="text-xl font-bold text-gray-900">{{ $order->order_control ?? '--' }}</div>
                    </div>
                    <div>
                        <div class="text-sm font-medium text-gray-500 mb-1">Data da Compra</div>
                        <div class="text-lg text-gray-900">{{ $order->created_at->format('d/m/Y H:i') }}</div>
                    </div>
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <div>
                        ...
                        <div class="text-base font-semibold text-gray-900 uppercase">{{ $order->buyer_name ?? '--' }}</div>
                    </div>
                    <div>
                        <div class="text-sm font-medium text-gray-500 mb-1">Email</div>
                        <div class="text-base text-gray-900 lowercase">{{ $order->buyer_email ?? '--' }}</div>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <div class="text-base text-gray-900">({{ $order->buyer_contact_ddd ?? '--' }}) {{ $order->buyer_contact_num ?? '--' }}</div>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    @forelse ($order->itens ?? [] as $orderKey => $orderItem)
                        <div class="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                            <div class="flex justify-between items-center">
                                <div class="flex-1">
                                    <div class="text-base font-medium text-gray-600">{{ $target->event_tickets_nomenclature ?? 'INGRESSO' }}</div>
                                    <div class="text-lg font-semibold text-gray-900 uppercase">{{ $orderItem->item_description ?? 'ND' }}</div>
                                    <div class="text-sm text-gray-600 mt-1 uppercase">- {{ $orderItem->user_name ?? 'PARTICIPANTE #' . ($orderKey + 1) }}</div>
                                </div>
                                <div class="text-xl font-bold text-gray-900">{{ toMoney($orderItem->item_amount ?? 0) }}</div>
                            </div>
                        </div>
                    @empty
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                @if ($order->order_cancel_datetime ?? false)
                                    <div class="text-sm text-red-600 mt-1">Cancelada em: {{ $order->order_cancel_datetime->format('d/m/Y H:i') }}</div>
                                @endif
                                @if ($order->order_cancel_description ?? false)
                                    <div class="text-sm text-red-700 mt-2">{{ $order->order_cancel_description }}</div>
                                @endif
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="space-y-4">
                            <div>
                                <x-input
                                    ...
                                    wire:model.defer="order_cancel_description"
                                />
                            </div>
                            <div class="flex justify-end">
                                <x-button
                                    ...
                                    right-icon="ban"
                                    onclick="confirm('Confirma o cancelamento da compra? Será irreversível!') || event.stopImmediatePropagation()"
                                    wire:click="cancelarOrderSubmit('{{ $order->id }}')"
                                    spinner
                                />
                            </div>
                        </div>
