...
class PagarmeServiceApiV5
{
    private $url;
    private $url_postback;
    private $gateway;
    private $customerGatewayId;
    private $mode;

    public function __construct($customerGatewayId, $mode='sandbox')
    {
        try {
            ...
            switch ($mode) {
                case 'sandbox':
                    $this->url = 'https://api.pagar.me/core/v5/';
                    $this->url_postback = route('payment-callback-gateway-slug', ['gatewaySlug' => 'pagarme-v5-sandbox' , 'callbackType' => 'PagarmeServiceApiV5']);
                    break;

                case 'live':
                    $this->url = 'https://api.pagar.me/core/v5/';
                    $this->url_postback = route('payment-callback-gateway-slug', ['gatewaySlug' => 'pagarme-v5-live' , 'callbackType' => 'PagarmeServiceApiV5']);
                    break;

                default:
                    throw new Exception('mode_not_found');
                    break;
            }

            $this->gateway = CustomerPayGateway::with('appGateway')->find($customerGatewayId);

            dd(
                $this->gateway->toArray(),
            );
        }
        catch (\Throwable $th)
        {
