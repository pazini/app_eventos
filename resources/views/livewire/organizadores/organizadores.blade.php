<div class="mb-10">
    ...
    @if ($customer ?? false)
            ...
            <div class="relative z-10 w-full space-y-6">
                <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
                    ...
                    <div class="flex flex-col sm:flex-row gap-4 w-full lg:w-auto">
                        ...
                        <div class="min-w-0 flex-1 sm:min-w-48">
                            ...
                            <x-native-select xs wire:model="organization_id" class="text-gray-700 w-full uppercase pt-2 pb-2 bg-white/95 backdrop-blur-sm border-white/30 focus:border-white focus:ring-2 focus:ring-white/50 rounded-lg shadow-sm transition-all duration-200">
                                <option value="">Selecione</option>
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <nav class="flex flex-wrap gap-2">
                        ...
                        <x-button white sm label="Setores" href="{{ route($context . '-organizadores-setores') }}" class="px-4 py-2 bg-white/95 text-sky-600 hover:bg-white hover:text-sky-700 transition-all duration-200 rounded-lg font-medium shadow-sm" />
        ...
        <x-modal.card title="Remover Organizador" blur wire:model.defer="removerOrganizer">
            ...
            @if ($this->organizer ?? false)
                ...
                <x-slot name="footer">
                    ...
                    @if(($context === 'campanhas' && $this->organizer->campaigns->count()) || ($context !== 'campanhas' && $this->organizer->events->count()))
        ...
        <x-modal.card title="{{ ($this->organizer ?? false) ? 'Alterar Organizador' : 'Novo Organizador' }}" blur wire:model.defer="novoOrganizer">
            ...
            @else
                ...
                <div class="grid grid-cols-1 md:grid-cols-6 gap-4">
                    ...
                    <div class="col-span-full">
                        <x-native-select label="Filial" wire:model="novoOrganizerOrganizationId" class="uppercase">
                            <option value="">--</option>
                        ...
                        @if ($this->organization_id ?? false)
                            @if ($context !== 'campanhas')
                                @if (($this->organizationsSub ?? false) && $this->organizationsSub->count())
                                    <x-native-select label="Setor" wire:model.defer="organization_sub_id" class="">
                                        <option value="">--</option>
                                        @foreach ($this->organizationsSub ?? [] as $item)
                                        <option value="{{ $item->id }}">{{ $item->organization_sub_name }} | {{ $item->organization_sub_description }}</option>
                                        @endforeach
                                    </x-native-select>
                                @else
                                    {!! setLabel(' ', "<span class='text-red-600 font-medium'>Não existem Setores disponíveis</span>") !!}
                                @endif
                            @endif
        ...
        <div class="{{ setClass('divContent') }} rounded-none border-x border-b">
            <div class="flex-1">
                <div class="{{ setClass('divContentTitle') }} uppercase mb-2">
                    <span class="font-light">Organizadores de {{ $context === 'campanhas' ? 'Campanhas' : 'Eventos' }}</span>
                    @if ($organization_id ?? false)
                        <span class="font-medium text-sky-600 ml-2">{{ strtolower($this->organizations->find($organization_id)->organization_name ?? '') }}</span>
                    @endif
                </div>
                @if ($organization_id ?? false)
                    <p class="text-gray-600 text-sm">Gerencie os organizadores de {{ $context === 'campanhas' ? 'campanhas' : 'eventos' }} desta filial</p>
                @endif
            </div>
        ...
        @if ($organization_id ?? false)
            <div class="w-full max-w-7xl mx-auto bg-white border-x border-b px-6 py-4">
                <div class="max-w-md flex gap-2 items-start">
                    <div class="flex-1">
                        <x-input
                ...
                @forelse ($organizers->sortBy('organizer_name') ?? [] as $organizer)
                    ...
                    <div class="bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden">
                        ...
                        <div class="p-6 grid grid-cols-12 gap-4">
                            ...
                            @if (session('organizer_status_' . $organizer->id))
                                <div class="col-span-12 mb-4">
                                    <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                                        <div class="flex items-center">
                                            ...
                                            <span class="text-blue-800 font-medium">{{ __(session('organizer_status_' . $organizer->id)) }}</span>
                                        </div>
                                    </div>
                                </div>
                            @endif

                            @if (session('organizer_error_' . $organizer->id))
                                <div class="col-span-12 mb-4">
                                    <div class="bg-red-50 border border-red-200 rounded-lg p-4">
                                        <div class="flex items-center">
                                            ...
                                            <span class="text-red-800 font-medium">{{ __(session('organizer_error_' . $organizer->id)) }}</span>
                                        </div>
                                    </div>
                                </div>
                            @endif
                            ...
                            <div class="col-span-12 border-b border-gray-100 pb-4 mb-4">
                                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                                    <div class="flex-1 space-y-2">
                                        ...
                                        <p class="text-sm text-gray-600 uppercase" title="organizationId: {{$organizer->organization_id ?? '--'}}@if($context !== 'campanhas') | organizationSubId: {{$organizer->organization_sub_id ?? '--'}}@endif">
                            ...
                            @if (session('associarUsuario_error_' . $organizer->id))
                                <div class="col-span-12 mb-4">
                                    <div class="bg-red-50 border border-red-200 rounded-lg p-4">
                                        <div class="flex items-center">
                                            <svg class="w-5 h-5 text-red-600 mr-3" fill="currentColor" viewBox="0 0 20 20">
                                            ...
                                            <span class="text-red-800 font-medium">{{ __(session('associarUsuario_error_' . $organizer->id)) }}</span>
                                        </div>
                                    </div>
                                </div>
                            @endif
                    ...
                    <div class="text-center py-12">
                        <div class="max-w-md mx-auto">
                            @if ($organization_id ?? false)
                                ...
                                <p class="text-gray-600 mb-6">Esta filial ainda não possui organizadores cadastrados.</p>
                            ...
                            @else
                                ...
                                <p class="text-gray-600">Escolha uma filial no filtro acima para visualizar seus organizadores.</p>
                            @endif
                        </div>
                    </div>
                @endforelse
    ...
    @elseif($customers ?? false)
        ...
        <div class="{{ setClass('divContentTitleDiv') }} rounded-t-lg">
            <div>
                <div class="{{ setClass('divContentTitle') }}">Organizadores</div>
