<div class="w-full max-w-7xl mx-auto mb-6">
                ...
                <div class="flex items-center space-x-2 flex-1 justify-center">
                    ...
                    <div>
                        <h1 class="flex flex-col md:flex-row gap-2 text-base md:text-3xl font-bold text-white">
                            ...
                            @if($target->event_name ?? false)
                                <div class="text-white/90 font-light">{{ $target->event_name }}</div>
                            @endif
                        </h1>
                    </div>
                </div>
        ...
        <div class="w-full max-w-2xl">
            ...
            <div class="bg-white shadow-sm border-x border-b rounded-b-lg mb-4" id="dataQrCode" style="display: none;">
                ...
                <div class="p-6 space-y-4">
                    ...
                    {{-- BOTÃO REALIZAR CHECKIN (se disponível) --}}
                    <div id="statusDisponivel" style="display: none;">
                        <x-button positive label="REALIZAR CHECKIN" onclick="realizaCheckin()" class="w-full py-3 text-base font-semibold" />
                    </div>

                    {{-- DATA/HORA DO CHECKIN (se já utilizado) --}}
                    ...
                    {{-- BOTÃO NOVO CHECKIN --}}
                    <div class="pt-2">
                        <x-button rounded outline primary label="NOVO CHECKIN" onclick="novoCheckin()" class="w-full py-3" />
                    </div>
                </div>
            </div>

            {{-- ÁREA DO LEITOR DE QR CODE --}}
            ...
            @if ($target->event_slug ?? false)
                <div class="mb-4">
                    <x-button href="{{ route('dashboard-vendas',['target_ref' => 'evento', 'target_slug' => $target->event_slug, 'target_id' => $target->id, 'view_status' => 'participantes']) }}" label="LISTA DE PARTICIPANTES" icon="clipboard-list" class="w-full py-3" />
                </div>
            @endif
    ...
    {{-- SCRIPTS --}}
    <script src="{{ asset('/js/mebjas_html5-qrcode_master_minified_html5-qrcode.min.js') }}"></script>
    ...
    <script type="text/javascript">
        ...
        function realizaCheckin()
        {
            if(confirm('Confirma o check-in? Esta ação é irreversível!'))
            {
                ...
                var url = '{{ config('domains.eventos') }}/api/checkin/event/' + qrCodeRealizar + '/realizar';
        ...
        function buscaCheckin(qrCodeCheckin)
        {
            ...
            if(qrCodeCheckin != buscaCheckinQrCode)
            {
                ...
                try
                {
                    ...
                    var url = '{{ config('domains.eventos') }}/api/checkin/event/' + qrCodeCheckin;
