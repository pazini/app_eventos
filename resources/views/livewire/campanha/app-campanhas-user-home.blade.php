<div class="min-h-screen bg-white">
                ...
                <section id="todas">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                        @foreach($campaignList as $campaign)
                            ...
                            <a href="{{ campanhaUrl($campaign->customer_organization_slug, $campaign->slug, null, $appUserUuid ?? null, $appSource ?? null) }}" class="group block">
    ...
    @if ($appUserUuid ?? false)
        <div class="fixed bottom-4 right-4 bg-white border border-gray-200 rounded-lg p-3 shadow-lg max-w-xs">
            <div class="flex items-center gap-2">
                <p class="text-xs text-gray-600 font-mono truncate flex-1" id="appUserUuid">{{ $appUserUuid }}</p>
                <button
                    onclick="copyToClipboard('{{ $appUserUuid }}')"
