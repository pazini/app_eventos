...
@push('meta')
    ...
    <meta name="title" content="{{ $campaignTitle }}">
    ...
    <meta property="og:site_name" content="{{ appName() }} - Campanhas"> {{-- White Label: nome dinâmico --}}
    ...
    <!-- Favicon -->
    @if ($campaignFavicon)
        <link rel="icon" type="image/png" href="{{ $campaignFavicon }}">
        <link rel="apple-touch-icon" href="{{ $campaignFavicon }}">
    @endif
@endpush
...
@push('styles')
    <style>
        :root {
            --campaign-color-primary: {{ $campaign->color_primary ?? '#3B82F6' }};
        }
...
<div id="div-title-campaing" class="min-h-screen pt-2 campaign-modern-surface"
    style="--color-primary: {{ $campaign->color_primary ?? '#3B82F6' }}; --color-secondary: {{ $campaign->color_secondary ?? '#10B981' }}; --color-primary-soft: {{ ($campaign->color_primary ?? '#3B82F6') . '2D' }}; --color-secondary-soft: {{ ($campaign->color_secondary ?? '#10B981') . '2D' }}; background-color: {{ $campaign->color_primary ?? '#3B82F6' }}15;">

    <div class="max-w-4xl mx-auto px-4 pt-4 relative z-10">

        {{-- Banner da campanha --}}
        @if (!in_array($step, [3]) && $campaign->url_image_banner)
            <div class="overflow-hidden" style="border-radius: 24px 24px 0 0;">
                <img src="{{ $getImageUrl($campaign->url_image_banner) }}" alt="{{ $campaign->name }}"
                    class="w-full h-auto block">
            </div>
        @endif

        {{-- Alerta de Status da Campanha --}}
        ...
        @if ($step !== 3)
            <div class="campaign-modern-panel campaign-modern-hero mb-6 p-6" style="border-radius: 0 0 24px 24px;">
                <div class="w-full flex flex-col md:flex-row justify-between items-start">
                    ...
                    <div>
                        <div class="text-xl md:text-3xl font-bold text-gray-900 uppercase">{{ $campaign->name }}</div>
                        @if ($campaign->organizer && ($campaign->organizer->organizer_name_full ?? $campaign->organizer->organizer_name))
                ...
                {{-- @if ($campaign->name_short)
                    <p class="text-sm text-gray-500 mt-1">{{ $campaign->name_short }}</p>
                @endif --}}
        ...
        @if ($campaign->show_goal_amount && $campaign->goal_amount && $step == 1)
            @php
            ...
            <div class="campaign-modern-panel p-6 mb-6">
                <div class="flex flex-col md:flex-row justify-between items-start gap-4 mb-4">
                    ...
                    @if ($campaign->datetime_start || $campaign->datetime_finish)
                        <div class="text-lg text-left md:text-right text-gray-600">
                            @if ($campaign->datetime_start)
                                <div>Desde {{ \Carbon\Carbon::parse($campaign->datetime_start)->format('d/m/Y') }}
                                </div>
                            @endif
                            @if ($campaign->datetime_finish)
                                <div>Até {{ \Carbon\Carbon::parse($campaign->datetime_finish)->format('d/m/Y') }}</div>
                            @endif
                        </div>
                    @endif
                </div>
                ...
                <div class="space-y-5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                        ...
                        <div class="campaign-progress-mini px-4 py-2 rounded-xl border"
                            style="border-color: {{ $campaign->color_primary }}33; background-color: {{ $campaign->color_primary }}0F;">
                            <p class="text-xs font-semibold uppercase tracking-wide"
                                style="color: {{ $campaign->color_primary ?? '#3B82F6' }}">Arrecadado</p>
                            <p class="text-3xl md:text-3xl font-bold"
                                style="color: {{ $campaign->color_primary ?? '#3B82F6' }}">
                                {{ toMoney($totalReceived, 'R$ ') }}
                            </p>
                            <p class="text-[11px] font-medium"
                                style="color: {{ $campaign->color_primary ?? '#3B82F6' }}">Arrecadamos até agora</p>
                        </div>

                        <div class="campaign-progress-mini px-4 py-2 rounded-xl border"
                            style="border-color: {{ $campaign->color_secondary }}33; background-color: {{ $campaign->color_secondary }}0F;">
                            <p class="text-xs font-semibold uppercase tracking-wide"
                                style="color: {{ $campaign->color_secondary ?? '#10B981' }}">Meta</p>
                            <p class="text-2xl md:text-3xl font-bold"
                                style="color: {{ $campaign->color_secondary ?? '#10B981' }}">
                                {{ toMoney($campaign->goal_amount, 'R$ ') }}
                            </p>
                            <p class="text-[11px] font-medium"
                                style="color: {{ $campaign->color_secondary ?? '#10B981' }}">Nosso Objetivo</p>
                        </div>
                    ...
                    @if ($campaign->show_progress)
                        <div class="space-y-2">
                            <div class="campaign-progress-track w-full rounded-full h-4 shadow-inner overflow-hidden"
                                style="background-color: {{ $campaign->color_primary }}33;">
                                @if ($percentReceived >= 100)
                                    <div class="h-4 rounded-full transition-all duration-500"
                                        style="width: 100%; background: linear-gradient(to right, {{ $campaign->color_primary ?? '#3B82F6' }}, {{ $campaign->color_secondary ?? '#10B981' }});">
                                    </div>
                                @else
                                    <div class="h-4 rounded-full transition-all duration-500"
                                        style="width: {{ $percentReceived }}%; background: linear-gradient(to right, {{ $campaign->color_primary ?? '#3B82F6' }}, {{ $campaign->color_secondary ?? '#10B981' }});">
                                    </div>
                                @endif
                            </div>
                            <div class="flex flex-col md:flex-row md:items-center md:justify-between text-sm gap-1">
                                <span class="font-semibold"
                                    style="color: {{ $campaign->color_primary ?? '#3B82F6' }}">
                                    {{ number_format($percentReceived, 0) }}% alcançado · {{ $totalAdesoes }}
                                    {{ $totalAdesoes == 1 ? 'adesão' : 'adesões' }}
                                </span>
                                <span class="text-slate-600 font-medium">
                                    Projeção restante: <span class="font-semibold"
                                        style="color: {{ $campaign->color_primary ?? '#3B82F6' }}">{{ toMoney($remaining, 'R$ ') }}</span>
                                </span>
                            </div>
                        </div>
                    @endif
        ...
        <div id="div-step" class="pt-4">
            ...
            {{-- Wizard de Participação --}}
            <div class="campaign-modern-shell">

                <div class="campaign-stage-content px-4 py-6 md:px-5">
                    @if (!in_array($campaign->status, ['active', 'active_direct']))

                        <x-jet-validation-errors class="mb-4" no_loading="true" />
                    ...
                    @else
                            ...
                            <div class="space-y-6 mt-2 campaign-step-card">
                                <div>
                                    <h2 class="text-2xl font-bold text-gray-900 mb-2">
                                        {{ $campaign->enable_questions && $campaign->questions->count() > 0 ? 'Antes de contribuir, responda algumas perguntas' : 'Escolha o valor' }}
                                    </h2>
                                    <p class="text-sm text-gray-600">
                                        {{ $campaign->enable_questions && $campaign->questions->count() > 0 ? 'Suas respostas nos ajudam a entender melhor nossa comunidade' : 'Toda contribuição faz diferença!' }}
                                    </p>
                                </div>

                                {{-- Perguntas do Quiz --}}
                                @if ($campaign->enable_questions && $campaign->questions->count() > 0)
                                    <div class="space-y-5">
                                        @foreach ($campaign->questions as $question)
                                            <div class="bg-gray-50 rounded-lg p-5 border border-gray-200">
                                                ...
                                                @if ($question->question_type === 'text')
                                                    <x-input wire:model.defer="quizAnswers.{{ $question->id }}"
                                                        placeholder="{{ $question->placeholder }}" class="w-full" />
                                                @elseif($question->question_type === 'textarea')
                                                    <x-textarea wire:model.defer="quizAnswers.{{ $question->id }}"
                                                        placeholder="{{ $question->placeholder }}" rows="4"
                                                        class="w-full" />
                                                @elseif($question->question_type === 'number')
                                                    <x-input type="number"
                                                        wire:model.defer="quizAnswers.{{ $question->id }}"
                                                        placeholder="{{ $question->placeholder }}" class="w-full" />
                                                ...
                                                @elseif($question->question_type === 'select')
                                                    <select wire:model.defer="quizAnswers.{{ $question->id }}"
                                                        class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                                        <option value="">
                                                            {{ $question->placeholder ?: 'Selecione...' }}</option>
                                                ...
                                                @elseif($question->question_type === 'radio')
                                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-2"
                                                        wire:key="radio-group-{{ $question->id }}">
                                                        @foreach ($question->question_options ?? [] as $option)
                                                            @php
                                                                $isSelected =
                                                            ...
                                                            @endphp
                                                            <label
                                                                wire:key="radio-{{ $question->id }}-{{ $loop->index }}"
                                                                wire:click="$set('quizAnswers.{{ $question->id }}', '{{ $option }}')"
                                                                class="flex items-center gap-3 p-3 border-2 rounded-lg cursor-pointer transition {{ $isSelected ? 'font-bold' : 'border-gray-300 bg-white' }}"
                                                                style="{{ $isSelected ? 'border-color: ' . ($campaign->color_primary ?? '#3B82F6') . '; background-color: ' . ($campaign->color_primary ?? '#3B82F6') . '0F;' : '' }} {{ !$isSelected ? '' : 'transition: border-color 0.3s;' }}"
                                                                onmouseover="if (!this.querySelector('input').checked) this.style.borderColor = '{{ $campaign->color_primary ?? '#3B82F6' }}66'"
                                                                ...
                                                                <div class="flex items-center gap-2 flex-1">
                                                                    <div class="w-5 h-5 rounded-full border-2 flex items-center justify-center transition {{ $isSelected ? '' : 'border-gray-400' }}"
                                                                        style="{{ $isSelected ? 'border-color: ' . ($campaign->color_primary ?? '#3B82F6') . '; background-color: ' . ($campaign->color_primary ?? '#3B82F6') . ';' : '' }}">
                                                                        @if ($isSelected)
                                                                    ...
                                                                    <span
                                                                        class="text-sm text-gray-900">{{ $option }}</span>
                                                                </div>
                                                            </label>
                                                        @endforeach
                                                    </div>
                                                @elseif($question->question_type === 'checkbox')
                                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-2"
                                                        x-data="{
                                                            selected: @js(isset($quizAnswers[$question->id]) && is_array($quizAnswers[$question->id]) ? $quizAnswers[$question->id] : []),
                                                            toggle(value) {
                                                                ...
                                                                @this.set('quizAnswers.{{ $question->id }}', [...this.selected]);
                                                            },
                                                            isSelected(value) {
                                                                return this.selected.includes(value);
                                                            }
                                                        }"
                                                        wire:key="checkbox-group-{{ $question->id }}">
                                                        @foreach ($question->question_options ?? [] as $option)
                                                            @php $optionValue = addslashes($option); @endphp
                                                            <div wire:key="checkbox-{{ $question->id }}-{{ $loop->index }}"
                                                                @click="toggle('{{ $optionValue }}')"
                                                                class="flex items-center gap-3 p-3 border-2 rounded-lg cursor-pointer transition"
                                                                :class="isSelected('{{ $optionValue }}') ? 'font-bold' :
                                                                    'border-gray-300 bg-white'"
                                                                :style="isSelected('{{ $optionValue }}') ?
                                                                    'border-color: {{ $campaign->color_primary ?? '#3B82F6' }}; background-color: {{ $campaign->color_primary ?? '#3B82F6' }}0F;' :
                                                                    ''"
                                                                @mouseover="if (!isSelected('{{ $optionValue }}')) $el.style.borderColor = '{{ $campaign->color_primary ?? '#3B82F6' }}66'"
                                                                @mouseout="if (!isSelected('{{ $optionValue }}')) $el.style.borderColor = '#D1D5DB'">
                                                                <input type="checkbox"
                                                                    wire:model="quizAnswers.{{ $question->id }}"
                                                                    value="{{ $option }}" class="hidden"
                                                                ...
                                                                <div class="flex items-center gap-2 flex-1">
                                                                    <div class="w-5 h-5 rounded border-2 flex items-center justify-center transition"
                                                                        :class="isSelected('{{ $optionValue }}') ? '' :
                                                                            'border-gray-400'"
                                                                        :style="isSelected('{{ $optionValue }}') ?
                                                                            'border-color: {{ $campaign->color_primary ?? '#3B82F6' }}; background-color: {{ $campaign->color_primary ?? '#3B82F6' }};' :
                                                                            ''">
                                                                        <svg class="w-4 h-4 text-white transition"
                                                                            fill="none" stroke="currentColor"
                                                                            viewBox="0 0 24 24"
                                                                            x-show="isSelected('{{ $optionValue }}')"
                                                                            style="display: none;">
                                                ...
                                                @if ($question->help_text)
                                                    <p class="text-xs text-gray-600 pt-3 flex items-start gap-1">
                                                        ...
                                                        <span class="italic">{{ $question->help_text }}</span>
                                                    </p>
                                                @endif

                                                @error('quiz_' . $question->id)
                                ...
                                <div>
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <div class="grid grid-cols-3 md:grid-cols-6 gap-2 md:gap-3">
                                            <button type="button" wire:click="setAmount(50)"
                                                class="px-4 py-3 rounded-full border-2 font-bold text-sm transition-all {{ $amount_total == 5000 ? '' : 'border-gray-300 bg-white text-gray-700' }}"
                                                style="{{ $amount_total == 5000 ? 'border-color: ' . ($campaign->color_primary ?? '#3B82F6') . '; background-color: ' . ($campaign->color_primary ?? '#3B82F6') . '0F; color: ' . ($campaign->color_primary ?? '#3B82F6') . ';' : '' }}"
                                                onmouseover="if ({{ $amount_total == 5000 ? 'false' : 'true' }}) this.style.borderColor = '{{ $campaign->color_primary ?? '#3B82F6' }}66'"
                                                onmouseout="if ({{ $amount_total == 5000 ? 'false' : 'true' }}) this.style.borderColor = '#D1D5DB'">
                                                R$ 50
                                            </button>
                                            <button type="button" wire:click="setAmount(100)"
                                                class="px-4 py-3 rounded-full border-2 font-bold text-sm transition-all {{ $amount_total == 10000 ? '' : 'border-gray-300 bg-white text-gray-700' }}"
                                                style="{{ $amount_total == 10000 ? 'border-color: ' . ($campaign->color_primary ?? '#3B82F6') . '; background-color: ' . ($campaign->color_primary ?? '#3B82F6') . '0F; color: ' . ($campaign->color_primary ?? '#3B82F6') . ';' : '' }}"
                                                onmouseover="if ({{ $amount_total == 10000 ? 'false' : 'true' }}) this.style.borderColor = '{{ $campaign->color_primary ?? '#3B82F6' }}66'"
                                                onmouseout="if ({{ $amount_total == 10000 ? 'false' : 'true' }}) this.style.borderColor = '#D1D5DB'">
                                                R$ 100
                                            </button>
                                            <button type="button" wire:click="setAmount(200)"
                                                class="px-4 py-3 rounded-full border-2 font-bold text-sm transition-all {{ $amount_total == 20000 ? '' : 'border-gray-300 bg-white text-gray-700' }}"
                                                style="{{ $amount_total == 20000 ? 'border-color: ' . ($campaign->color_primary ?? '#3B82F6') . '; background-color: ' . ($campaign->color_primary ?? '#3B82F6') . '0F; color: ' . ($campaign->color_primary ?? '#3B82F6') . ';' : '' }}"
                                                onmouseover="if ({{ $amount_total == 20000 ? 'false' : 'true' }}) this.style.borderColor = '{{ $campaign->color_primary ?? '#3B82F6' }}66'"
                                                onmouseout="if ({{ $amount_total == 20000 ? 'false' : 'true' }}) this.style.borderColor = '#D1D5DB'">
                                                R$ 200
                                            </button>
                                            <button type="button" wire:click="setAmount(250)"
                                                class="px-4 py-3 rounded-full border-2 font-bold text-sm transition-all {{ $amount_total == 25000 ? '' : 'border-gray-300 bg-white text-gray-700' }}"
                                                style="{{ $amount_total == 25000 ? 'border-color: ' . ($campaign->color_primary ?? '#3B82F6') . '; background-color: ' . ($campaign->color_primary ?? '#3B82F6') . '0F; color: ' . ($campaign->color_primary ?? '#3B82F6') . ';' : '' }}"
                                                onmouseover="if ({{ $amount_total == 25000 ? 'false' : 'true' }}) this.style.borderColor = '{{ $campaign->color_primary ?? '#3B82F6' }}66'"
                                                onmouseout="if ({{ $amount_total == 25000 ? 'false' : 'true' }}) this.style.borderColor = '#D1D5DB'">
                                                R$ 250
                                            </button>
                                            <button type="button" wire:click="setAmount(500)"
                                                class="px-4 py-3 rounded-full border-2 font-bold text-sm transition-all {{ $amount_total == 50000 ? '' : 'border-gray-300 bg-white text-gray-700' }}"
                                                style="{{ $amount_total == 50000 ? 'border-color: ' . ($campaign->color_primary ?? '#3B82F6') . '; background-color: ' . ($campaign->color_primary ?? '#3B82F6') . '0F; color: ' . ($campaign->color_primary ?? '#3B82F6') . ';' : '' }}"
                                                onmouseover="if ({{ $amount_total == 50000 ? 'false' : 'true' }}) this.style.borderColor = '{{ $campaign->color_primary ?? '#3B82F6' }}66'"
                                                onmouseout="if ({{ $amount_total == 50000 ? 'false' : 'true' }}) this.style.borderColor = '#D1D5DB'">
                                                R$ 500
                                            </button>
                                            <button type="button" wire:click="setAmount(750)"
                                                class="px-4 py-3 rounded-full border-2 font-bold text-sm transition-all {{ $amount_total == 75000 ? '' : 'border-gray-300 bg-white text-gray-700' }}"
                                                style="{{ $amount_total == 75000 ? 'border-color: ' . ($campaign->color_primary ?? '#3B82F6') . '; background-color: ' . ($campaign->color_primary ?? '#3B82F6') . '0F; color: ' . ($campaign->color_primary ?? '#3B82F6') . ';' : '' }}"
                                                onmouseover="if ({{ $amount_total == 75000 ? 'false' : 'true' }}) this.style.borderColor = '{{ $campaign->color_primary ?? '#3B82F6' }}66'"
                                                onmouseout="if ({{ $amount_total == 75000 ? 'false' : 'true' }}) this.style.borderColor = '#D1D5DB'">
                                                R$ 750
                                            </button>
                                        </div>
                                    </div>

                                    {{-- Valor Manual + Recorrência --}}
                                    @php
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-3 items-start">
                                        <div class="{{ $showRecurringOption ? 'md:col-span-1' : 'md:col-span-3' }}">
                                            <div class="campaign-donation-pane">
                                                ...
                                                <div class="campaign-currency-input flex items-stretch focus-within:ring-2 focus-within:border-transparent"
                                                    style="--tw-ring-color: {{ $campaign->color_primary ?? '#3B82F6' }};"
                                                    x-data="currencyField(@entangle('amount_total_input').defer)" x-init="init()">
                                                    <span
                                                ...
                                                <p class="text-[10px] text-gray-500 mt-1">
                                                    Valor mínimo: R$ {{ toMoney($campaign->amount_min) }}
                                                </p>
                                                @error('amount_total')
                                                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                                                @enderror
                                            ...
                                            <div class="h-full md:col-span-2">
                                                <div class="campaign-donation-pane">
                                                    ...
                                                    <div class="campaign-recurring-options">
                                                        <label
                                                            class="campaign-recurring-option flex items-center gap-3 w-full border px-4 py-3.5 text-sm font-semibold transition cursor-pointer {{ $recurringAvailable ? '' : 'opacity-50 cursor-not-allowed' }}"
                                                            style="{{ $is_recurring === true ? 'border-color: ' . ($campaign->color_primary ?? '#3B82F6') . '; background-color: ' . ($campaign->color_primary ?? '#3B82F6') . '0F; color: ' . ($campaign->color_primary ?? '#3B82F6') . ';' : 'border-color: #D1D5DB; color: #374151; background-color: #ffffff;' }}"
                                                            @if ($recurringAvailable) wire:click="$set('is_recurring', true)" @endif>
                                                            <input type="radio" name="recurring_choice"
                                                                class="h-5 w-5 text-blue-600 focus:ring-blue-500"
                                                                @if ($is_recurring === true) checked @endif
                                                                @if (!$recurringAvailable) disabled @endif />
                                                        ...
                                                        <label
                                                            ...
                                                            style="{{ $is_recurring === false ? 'border-color: #111827; background-color: #111827; color: #ffffff;' : 'border-color: #D1D5DB; color: #374151; background-color: #ffffff;' }}"
                                                            wire:click="$set('is_recurring', false)">
                                                            <input type="radio" name="recurring_choice"
                                                                class="h-5 w-5 text-blue-600 focus:ring-blue-500"
                                                                @if ($is_recurring === false) checked @endif />
                                                    ...
                                                    @error('is_recurring')
                                                        <div class="mt-2 text-xs text-red-600">{{ $message }}
                                                        </div>
                                                    @enderror
                        ...
                        @if ($step === 2)
                            ...
                            <div class="space-y-6 campaign-step-card"
                                wire:key="step-2-form-{{ $order->id ?? 'new' }}">
                                {{-- Valor da Doação --}}
                                    ...
                                    @if ($amount_total ?? false)
                                        <div class="w-full md:w-auto flex justify-center md:justify-end">
                                            <div class="campaign-amount-shell"
                                                style="
                                                    --amount-primary: {{ $campaign->color_primary ?? '#3B82F6' }};
                                                    --amount-secondary: {{ $campaign->color_secondary ?? '#10B981' }};
                                                    --amount-primary-soft: {{ ($campaign->color_primary ?? '#3B82F6') . '22' }};
                                                    --amount-secondary-soft: {{ ($campaign->color_secondary ?? '#10B981') . '26' }};
                                                ">
                                                <div class="campaign-amount-pill">
                                                    <span class="campaign-amount-label">Contribuição</span>
                                                    <span
                                                        class="campaign-amount-value">{{ toMoney($amount_total ?? 0, 'R$ ') }}</span>
                                                </div>
                                            </div>
                                        </div>
                                    @endif
                                ...
                                @if ($campaign->allow_anonymous)
                                    <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                                        <div class="flex items-center justify-between">
                                            ...
                                            <x-toggle wire:model="is_anonymous" />
                                        </div>
                                    </div>
                                @endif
                                ...
                                @if (!($is_anonymous ?? false))
                                    <div>
                                        ...
                                        {{-- Linha 1: Telefone (DDI + DDD + Número) e Data de Nascimento --}}
                                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-start mb-4">
                                                    ...
                                                    <div class="col-span-12 md:col-span-4">
                                                        ...
                                                        <select wire:model="buyer_contact_country"
                                                            ...
                                                            @foreach ($ddiList as $ddiValue => $ddiLabel)
                                                                <option value="{{ $ddiValue }}">
                                                                    {{ $ddiLabel }}</option>
                                                            @endforeach
                                                        </select>
                                                    </div>

                                                    {{-- DDD (só aparece se for Brasil) --}}
                                                    @if ($buyer_contact_country === '55')
                                                        <div class="col-span-4 md:col-span-3">
                                                            <label
                                                                class="block text-xs font-semibold text-gray-700 uppercase mb-2">DDD
                                                                *</label>
                                                            <x-input type="tel" inputmode="numeric"
                                                                pattern="[0-9]*" wire:model="buyer_contact_ddd"
                                                        ...
                                                        <div class="col-span-8 md:col-span-5">
                                                            <label
                                                                ...
                                                                *</label>
                                                            <x-input type="tel" inputmode="numeric"
                                                                pattern="[0-9]*" wire:model="buyer_contact_num"
                                            ...
                                            <div class="col-span-12 md:col-span-1">
                                                ...
                                                <x-input type="date" inputmode="numeric" pattern="[0-9]*"
                                                    wire:model="buyer_birth_date" class="w-full" />
                                            </div>
                                        </div>
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                            <div>
                                                <label
                                                    ...
                                                    Completo *</label>
                                                <x-input wire:model="buyer_name" placeholder="Seu nome completo"
                                                    class="w-full" />
                                            </div>
                                            ...
                                            <div>
                                                ...
                                                <x-input type="email" wire:model="buyer_email"
                                            ...
                                            @if ($campaign->require_doc)
                                                <div>
                                                    ...
                                                    <x-input type="tel" inputmode="numeric" pattern="[0-9]*"
                                                        wire:model="buyer_doc_num" placeholder="Somente números"
                                                        maxlength="20"
                                                        oninput="this.value = this.value.replace(/[^0-9]/g, '')"
                                                        class="w-full" />
                                                </div>
                        ...
                        @if ($step === 3)
                            ...
                            <x-jet-validation-errors class="mb-4" no_loading="false" />

                            {{-- Alertas de Forma de Pagamento (sucesso, erro, info, warning) --}}
                            <div class="px-4">@include('_includes.alertas_forma_pagamento')</div>
                            ...
                            <div class="space-y-6 campaign-step-card"
                                @if ($order ?? false) data-order-id="{{ $order->id }}" @endif
                                wire:key="payment-step-{{ $order->id ?? 'new' }}">
                                ...
                                <div class="py-6 border-t border-b">
                                    ...
                                    <div class="mb-4 pb-4 border-b">
                                        ...
                                        @elseif($campaign->about)
                                            <div class="text-sm text-gray-700 line-clamp-3">
                                                {{ strip_tags($campaign->about) }}
                                            </div>
                                        @endif
                                    </div>
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <div class="flex justify-between text-sm">
                                            ...
                                            <span class="font-semibold uppercase">{{ $buyer_name }}</span>
                                        </div>
                                        <div class="flex justify-between text-sm">
                                            <span class="text-gray-700">Telefone</span>
                                            <span class="font-semibold uppercase">{{ $buyer_contact_country }}
                                                {{ $buyer_contact_ddd ? $buyer_contact_ddd . ' ' : null }}{{ $buyer_contact_num }}</span>
                                        </div>
                                        <div class="flex justify-between text-sm">
                                            <span class="text-gray-700">E-mail</span>
                                            <span class="font-semibold">{{ $buyer_email }}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="mt-4 pt-4 border-t">
                                        <div class="flex justify-between items-center">
                                            ...
                                            <span class="text-3xl font-bold"
                                                style="color: {{ $campaign->color_primary ?? '#3B82F6' }}">
                                                {{ toMoney($amount_total ?? 0, 'R$ ') }}
                                            </span>
                                        </div>
                                        @if ($payment_method === 'credit_card' && ($pay_installments_number ?? 1) > 1)
                                            @php
                                            ...
                                            <div class="flex justify-end mt-1">
                                                <span class="text-xs text-gray-500">
                                                    {{ $pay_installments_number }}x de
                                                    {{ toMoney($installmentValue, 'R$ ') }}
                                                </span>
                                            </div>
                                        @endif
                                    </div>
                                ...
                                {{-- Alerta: Doação Anônima só aceita PIX --}}
                                @if ($is_anonymous)
                                ...
                                @endphp
                                @if ($isRecurringPayment && !$hasRecurringMethod)
                                ...
                                @elseif(!$isRecurringPayment && !$hasAnyMethod)
                                    <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-center">
                                        <p class="text-sm text-yellow-800">
                                ...
                                @else
                                    <div class="space-y-3" id="payment-methods-container">
                                        ...
                                        {{-- PIX --}}
                                        @if (!$isRecurringPayment && ($campaign->pay_pix ?? false))
                                            @php
                                            ...
                                            @endphp
                                            <div class="border-2 rounded-lg overflow-hidden transition relative {{ $shouldOpenPix ? 'border-blue-500' : 'border-gray-300' }}"
                                                x-data="{ isOpen: {{ $shouldOpenPix ? 'true' : 'false' }} }" x-init="isOpen = {{ $shouldOpenPix ? 'true' : 'false' }}"
                                                x-effect="if ($wire.payment_method === 'pix') { isOpen = true; } else if ($wire.payment_method !== 'pix' && $wire.payment_method !== '') { isOpen = false; }">
                                                {{-- Loading overlay específico do card removido (usamos apenas o global para evitar 4 loaders simultâneos) --}}

                                                <label
                                                    class="flex items-center gap-4 p-4 cursor-pointer hover:bg-gray-50 transition relative {{ $shouldOpenPix ? 'bg-blue-50 font-bold' : '' }}"
                                                    ...
                                                    <input type="radio" wire:model="payment_method" value="pix"
                                                        @if ($shouldOpenPix) checked @endif
                                                ...
                                                <div x-show="isOpen"
                                                    ...
                                                    <div class="p-6 bg-white space-y-4">
                                                        ...
                                                        @if (!$hasPixGenerated || $isPixExpired)
                                                            <h3 class="text-lg font-bold text-gray-900">Dados para PIX
                                                            </h3>
                                                            <x-inputs.maskable label="CPF do Pagador *"
                                                                placeholder="000.000.000-00"
                                                                mask="['###.###.###-##','##.###.###/####-##']"
                                                                wire:model.defer="pix_cpf" wire:key="pix_cpf_maskable"
                                                            ...
                                                            <div class="flex justify-between gap-3 pt-4 border-t">
                                                                ...
                                                                <div>
                                                                    <button wire:click="processarPagamento"
                                                                        ...
                                                                        style="background-color: {{ $campaign->color_primary ?? '#3B82F6' }}">
                                                        ...
                                                        @else
                                                            <div class="space-y-2">
                                                                {{-- Polling automático para validar pagamento a cada 5 segundos (sem exibir mensagens) --}}
                                                                @if (
                                                                    $currentPayment &&
                                                                ...
                                                                <div
                                                                    ...
                                                                    @if ($payment->pay_pix_qr_code_url ?? ($payment_result['pay_pix_qr_code_url'] ?? false))
                                                                        <div class="md:col-span-1 flex justify-center">
                                                                            <img src="{{ $payment->pay_pix_qr_code_url ?? $payment_result['pay_pix_qr_code_url'] }}"
                                                                                alt="QR Code PIX"
                                                                    ...
                                                                    <div class="md:col-span-2 space-y-3">
                                                                        ...
                                                                        <div>
                                                                            ...
                                                                            <div
                                                                                ...
                                                                                {{ $payment->pay_pix_key ?? ($payment_result['pay_pix_key'] ?? '-') }}
                                                                            </div>
                                                                        </div>
                                                                        <button type="button"
                                                                            onclick="navigator.clipboard.writeText('{{ $payment->pay_pix_key ?? ($payment_result['pay_pix_key'] ?? '') }}').then(() => { alert('Código PIX copiado!'); }).catch(() => { alert('Erro ao copiar. Tente novamente.'); });"
                                                                        ...
                                                                        @if ($payment->pay_pix_expires_at ?? ($payment_result['pay_pix_expires_at'] ?? false))
                                                                            @php
                                        ...
                                        @if (!$isRecurringPayment && ($campaign->pay_pix_direto ?? false))
                                            @php
                                            ...
                                            <div class="border-2 rounded-lg overflow-hidden transition relative {{ $payment_method === 'pix_direto' || $hasPixDiretoGenerated ? 'border-purple-500' : 'border-gray-300' }}"
                                                x-data="{ isOpen: {{ $shouldOpenPixDireto ? 'true' : 'false' }} }"
                                                x-effect="isOpen = ($wire.payment_method === 'pix_direto')">

                                                <label
                                                    class="flex items-center gap-4 p-4 cursor-pointer hover:bg-gray-50 transition {{ $payment_method === 'pix_direto' || $hasPixDiretoGenerated ? 'bg-purple-50 font-bold' : '' }}"
                                                ...
                                                <div x-show="isOpen"
                                                    ...
                                                    class="border-t border-gray-200">
                                                    <div class="p-6 bg-white space-y-4">
                                                        @if ($hasPixDiretoGenerated)
                                                            {{-- QR Code e Key do PIX Direto --}}
                                                            <div class="text-center space-y-4">
                                                                @if ($currentPaymentPixDireto->pay_pix_qr_code_url)
                                                                    <div class="flex justify-center">
                                                                        <img src="{{ $currentPaymentPixDireto->pay_pix_qr_code_url }}"
                                                                            alt="QR Code PIX"
                                                                            class="w-64 h-64 border-4 border-purple-500 rounded-lg shadow-lg">
                                                                    </div>
                                                                @endif

                                                                @if ($currentPaymentPixDireto->pay_pix_key)
                                                                    <div>
                                                                        <div
                                                                        ...
                                                                        <div class="relative">
                                                                            <input type="text"
                                                                                value="{{ $currentPaymentPixDireto->pay_pix_key }}"
                                                                                readonly
                                                                                class="w-full px-4 py-2 pr-20 border border-gray-300 rounded-lg text-xs font-mono bg-gray-50"
                                                                                id="pix-direto-key">
                                                                            <button
                                                                                onclick="
                                                                                navigator.clipboard.writeText('{{ $currentPaymentPixDireto->pay_pix_key }}');
                                        ...
                                        {{-- Cartão de Crédito (desabilitado para doações anônimas) --}}
                                        @if (($campaign->pay_card_credit ?? false) && !$is_anonymous)
                                            <div class="border-2 rounded-lg overflow-hidden transition relative {{ $payment_method === 'credit_card' ? 'border-blue-500' : 'border-gray-300' }}"
                                                x-data="{ isOpen: @entangle('payment_method').live === 'credit_card' }"
                                                x-effect="isOpen = $wire.payment_method === 'credit_card'">
                                                {{-- Loading overlay específico do card removido (usamos apenas o global para evitar 4 loaders simultâneos) --}}

                                                <label
                                                    class="flex items-center gap-4 p-4 cursor-pointer hover:bg-gray-50 transition relative {{ $payment_method === 'credit_card' ? 'bg-blue-50 font-bold' : '' }}"
                                                    wire:loading.attr="disabled" wire:target="payment_method"
                                                    ...
                                                    <div class="flex items-center gap-3 flex-1">
                                                        ...
                                                        <div>
                                                            ...
                                                            <div class="text-xs text-gray-600">
                                                                ...
                                                                @else
                                                                    Parcelamento em até
                                                                    {{ $campaign->pay_card_credit_installment_max ?? 1 }}x
                                                                @endif
                                                            </div>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="p-6 bg-white space-y-4">
                                                        ...
                                                        <div class="grid grid-cols-6 gap-4">
                                                            <div class="col-span-full md:col-span-3">
                                                                <x-input wire:model="card_credit_nome"
                                                                    label="Nome no Cartão *"
                                                                    placeholder="IMPRESSO NO CARTÃO"
                                                                    class="uppercase" />
                                                            </div>
                                                            <div class="col-span-full md:col-span-3">
                                                                <x-inputs.maskable label="Número do Cartão *"
                                                                    placeholder="0000 0000 0000 0000"
                                                                    mask="['#### #### #### ####','#### #### #### #### ###']"
                                                                    wire:model.defer="card_credit_num"
                                                                    wire:key="card_credit_num_maskable"
                                                                    inputmode="numeric" required />
                                                            </div>
                                                            <div class="col-span-full md:col-span-3">
                                                                <label
                                                                ...
                                                                <div class="flex gap-2 w-full">
                                                                    <div class="w-1/2">
                                                                        <x-native-select
                                                                            wire:model="card_credit_validade_mm"
                                                                    ...
                                                                    <div class="w-1/2">
                                                                        <x-native-select
                                                                            wire:model="card_credit_validade_aaaa"
                                                                            class="w-full rounded-none">
                                                            ...
                                                            <div class="col-span-3 md:col-span-1">
                                                                <x-input wire:model="card_credit_cvv" label="CVV *"
                                                                    placeholder="000" type="tel"
                                                                    inputmode="numeric" pattern="[0-9]*"
                                                                    oninput="this.value = this.value.replace(/[^0-9]/g, '').slice(0, 4)"
                                                                    maxlength="4" class="w-full" />
                                                            </div>

                                                            <div class="col-span-full md:col-span-2">
                                                                <x-inputs.maskable label="CPF / CNPJ do Titular *"
                                                                    mask="['###.###.###-##','##.###.###/####-##']"
                                                                    placeholder="000.000.000-00"
                                                                    wire:model.defer="card_credit_cpf"
                                                                    wire:key="card_credit_cpf_maskable"
                                                                    inputmode="numeric" required />
                                                            </div>

                                                            <div class="col-span-full md:col-span-3">
                                                                @if ($is_recurring)
                                                                    <x-input label="Parcelas *" value="1x - À vista"
                                                                        readonly class="rounded-none bg-gray-50" />
                                                                @else
                                                                    <x-native-select label="Parcelas *"
                                                                        wire:model="pay_installments_number"
                                                                        class="rounded-none"
                                                                        wire:loading.class="opacity-50">
                                                                        @if ($payment_method === 'credit_card')
                                                                            <option value="">Selecione...
                                                                            </option>
                                                                            @if (!empty($available_installments) && is_array($available_installments) && count($available_installments) > 0)
                                                                                @foreach ($available_installments as $installment)
                                                                                    @if (is_array($installment) && isset($installment['installments']))
                                                                                        <option
                                                                                            value="{{ $installment['installments'] }}">
                                                                                            {{ $installment['label'] ?? $installment['installments'] . 'x' }}
                                                                                        </option>
                                                                                    @endif
                                                                                @endforeach
                                                                            @else
                                                                                ...
                                                                                @for ($i = 1; $i <= ($campaign->pay_card_credit_installment_max ?? 1); $i++)
                                                                                    <option
                                                                                        value="{{ $i }}">
                                                                                        {{ $i }}x
                                                                                        @if ($i > 1 && $campaign->pay_card_credit_installment_fee_payer === 'customer')
                                                                                            - COM JUROS
                                                                                        @endif
                                                                                    </option>
                                                                                @endfor
                                                                    ...
                                                                    @if (empty($available_installments) && $payment_method === 'credit_card')
                                        ...
                                        {{-- Boleto (desabilitado para doações anônimas) --}}
                                        @if (!$isRecurringPayment && ($campaign->pay_boleto ?? false) && !$is_anonymous)
                                            @php
                                            ...
                                            @endphp
                                            <div class="border-2 rounded-lg overflow-hidden transition relative {{ $payment_method === 'boleto' || $hasBoletoGenerated ? 'border-blue-500' : 'border-gray-300' }}"
                                                x-data="{ isOpen: {{ $shouldOpenBoleto ? 'true' : 'false' }} }" x-init="if ({{ $hasBoletoGenerated ? 'true' : 'false' }} || $wire.payment_method === 'boleto') {
                                                    isOpen = true;
                                                }"
                                                ...
                                                <label
                                                    class="flex items-center gap-4 p-4 cursor-pointer hover:bg-gray-50 transition relative {{ $payment_method === 'boleto' || $hasBoletoGenerated ? 'bg-blue-50 font-bold' : '' }}"
                                                    wire:loading.attr="disabled" wire:target="payment_method"
                                                    @click="$wire.set('payment_method', 'boleto')">
                                                    <input type="radio" wire:model="payment_method" value="boleto"
                                                        @if ($payment_method === 'boleto' || $hasBoletoGenerated) checked @endif
                                                        class="text-blue-600 focus:ring-blue-500" />
                                                    <div class="flex items-center gap-3 flex-1">
                                                ...
                                                <div x-show="isOpen"
                                                    ...
                                                    <div class="p-6 bg-white">
                                                        ...
                                                        @if (!$hasBoletoGenerated)
                                                            <h3 class="text-lg font-bold text-gray-900 mb-3">Dados para
                                                                Boleto</h3>
                                                            <x-inputs.maskable label="CPF do Pagador *"
                                                                placeholder="000.000.000-00"
                                                                hint="Banco Central exige desde 2017"
                                                                mask="['###.###.###-##','##.###.###/####-##']"
                                                                wire:model.defer="boleto_cpf"
                                                                wire:key="boleto_cpf_maskable" inputmode="numeric"
                                                                required />

                                                            {{-- Botão de ação Boleto --}}
                                                                ...
                                                                <div
                                                                    ...
                                                                    <div class="text-left md:text-right">
                                                                        ...
                                                                        <div class="text-xl font-bold text-blue-700">
                                                                            {{ \Carbon\Carbon::parse($boletoExpiration)->format('d/m/Y') }}
                                                                        </div>
                                                                    </div>
                            ...
                            @else
                                <div id="payment-confirmation" class="space-y-6 text-center campaign-step-card"
                                    ...
                                    @if (
                                        ...
                                        <div>
                                            <h2 class="text-2xl md:text-3xl font-bold text-gray-900 mb-2">
                                                {{ appText('messages.success.payment', 'Pagamento Confirmado!') }}
                                            </h2>
                                            <p class="text-lg md:text-lg text-gray-600">
                                                {{ appText('campaigns.thank_you', 'Obrigado por apoiar nossa campanha') }}
                                            </p>
                                        </div>
                                    @elseif($payment->pay_type === 'pix' && $payment->pay_pix_qr_code)
                                        {{-- PIX QR Code --}}
                                        <div>
                                        ...
                                        <div class="bg-white rounded-lg p-6 border-2 border-gray-200 inline-block">
                                            <img src="{{ $payment->pay_pix_qr_code_url ?? 'data:image/png;base64,' . $payment->pay_pix_qr_code }}"
                                                alt="QR Code PIX" class="w-64 h-64 mx-auto mb-4">
                                            @if ($payment->pay_pix_expires_at ?? false)
                                                @php
                                                ...
                                                @if ($minutesRemaining > 0 && $minutesRemaining <= 120)
                                                    <p class="text-sm text-orange-600 font-semibold">
                                                        ⚠️ Válido até: {{ $pixExpiresAt->format('d/m/Y H:i') }}
                                                    </p>
                                                @endif
                                            @endif
                                        </div>

                                        @if ($payment->pay_pix_key)
                                            <div
                                                ...
                                                <div class="flex items-center gap-2">
                                                    <input type="text" value="{{ $payment->pay_pix_key }}"
                                                        readonly
                                    ...
                                    @else
                                        ...
                                        @elseif($hasError)
                                            {{-- Quando há erro, não mostra tela de confirmação - volta para step 3 --}}
                                            {{-- Esta seção só aparece se por algum motivo o step for 4 com erro --}}
                                            <div
                                        ...
                                        <div class="">
                                            <div
                                                ...
                                                <span class="font-medium text-gray-900 capitalize"
                                                    style="color: {{ $campaign->color_primary ?? '#3B82F6' }}">{{ $order->order_control }}</span>
                                            </div>
                                            ...
                                            <div
                                                ...
                                                <span
                                                    class="font-medium text-gray-900 capitalize">{{ $order->buyer_name }}</span>
                                            </div>
                                            ...
                                            @if ($order->buyer_email)
                                                <div
                                                    ...
                                                    <span
                                                        class="font-medium text-gray-900 lowercase">{{ $order->buyer_email }}</span>
                                                </div>
                                            @endif
                                            @if ($order->buyer_contact_ddd || $order->buyer_contact_num)
                                                ...
                                                <div
                                                    ...
                                                    <span class="font-medium text-gray-900">+{{ $contactCountry }}
                                                        {{ $order->buyer_contact_ddd ? $order->buyer_contact_ddd . ' ' : '' }}{{ $order->buyer_contact_num }}</span>
                                                </div>
                                            @endif
                                        </div>
                                    </div>
                                        ...
                                        <div class="space-y-3 py-4">
                                            ...
                                            <div
                                                ...
                                                <span class="font-medium text-gray-900 text-sm uppercase">
                                                    @if ($payment->pay_type === 'pix')
                                                        PIX
                                                    @elseif($payment->pay_type === 'boleto')
                                                        BOLETO
                                            ...
                                            <div
                                                ...
                                                <span
                                                    class="font-medium text-gray-900 text-sm">{{ $payment->created_at ? $payment->created_at->format('d/m/Y H:i') : '--' }}</span>
                                            </div>
                                            {{-- NSU --}}
                                            <div
                                                class="flex flex-col md:flex-row justify-start md:justify-between items-start md:items-center px-4 py-2 rounded bg-gray-50 shadow">
                                                ...
                                                <span
                                                    class="font-medium text-gray-900 text-sm font-mono">{{ $payment->pay_nsu ?? '--' }}</span>
                                            </div>
                                            {{-- Dados do Cartão (extrair do JSON se necessário) --}}
                                            @php
                                            ...
                                            @if ($cardNumber || $cardBrand)
                                                ...
                                                <div
                                                    ...
                                                    <span
                                                        class="font-medium text-gray-900 text-sm font-mono uppercase">{{ $cardBrand ?? '--' }}</span>
                                                </div>
                                            @endif
                                        </div>
            ...
            <div class="text-center text-xs text-gray-400 mb-6">
                @if ($order ?? false)
                    <span>Localizador: <span
                            class="font-mono font-semibold text-gray-500">{{ $order->order_control }}</span></span>
                @elseif(session()->get('appUserUuid') ?? false)
                    <span>AppSession: <span
                            class="font-mono font-semibold text-gray-500">{{ session()->get('appUserUuid') ?? null }}</span></span>
                @endif
            </div>
        </div>
    ...
    {{-- Script para scroll automático ao avançar steps --}}
    <script>
        (function() {
...
@once
    <script>
        ...
        function getDadosCompartilhamentoCampanha() {
            ...
            const titulo = '{{ addslashes($campaign->name) }}';
            const descricao =
                '{{ addslashes(Str::limit(strip_tags($campaign->description ?? 'Participe desta campanha!'), 110)) }}';
