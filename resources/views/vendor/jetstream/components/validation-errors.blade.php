        ...
        <div class="{{ setClass('divContentErros') }}">
            @if (count($errors->all()) > 1)
                <div class="flex justify-center w-full mx-auto bg-red-600 text-white p-3 my-1 rounded-sm shadow-md">
                    <div class="w-full m-0 px-2 text-white uppercase text-lg">{{ count($errors->all()) }} erros foram encontrados</p>
                </div>
            @else
                @foreach ($errors->all() as $error)
                    <div class="flex w-full mx-auto bg-red-600 text-white p-3 my-1 rounded-sm shadow-md">
                        <p class="w-full m-0 px-2 text-white uppercase text-lg">{{ $error }}</p>
                    </div>
                @endforeach
            @endif
        </div>
