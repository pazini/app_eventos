...
<div class="grid grid-cols-[4px_repeat(7,1fr)] items-center bg-white border-t border-gray-100 px-3 py-2.5 gap-x-3">
    ...
    <div class="self-stretch rounded-full" style="background:<?php echo e($pcBarColor); ?>;width:4px;"></div>

    
    <div>
        <?php if($pcDate): ?>
            <div class="text-xs font-semibold text-gray-700"><?php echo e($pcDate->format('d/m/Y H:i')); ?></div>
        <?php else: ?>
    ...
    <div class="col-span-2">
        <span class="inline-flex items-center text-[10px] font-bold uppercase px-2 py-0.5 rounded-full whitespace-nowrap"
              style="background:<?php echo e($pcColor['bg']); ?>;color:<?php echo e($pcColor['text']); ?>;">
            <?php echo e(__($pcStatus)); ?>

        </span>
        <?php if(in_array($paymentItem->pay_integration_type ?? '', ['sandbox'])): ?>
    ...
    <div class="text-[10px] font-mono text-gray-400 truncate">
        <?php echo e($paymentItem->pay_nsu ?? '—'); ?>

    </div>

    
    <div class="text-right">
        <?php if($paymentItem->value_paid ?? false): ?>
            <div class="text-sm font-extrabold text-gray-800"><?php echo e(toMoney($paymentItem->value_paid, 'R$ ')); ?></div>
            <?php if(($paymentItem->fee_percentage_used ?? 0) > 0): ?>
                <div class="text-[9px] text-gray-400">liq. <?php echo e(toMoney($paymentItem->value_liquid ?? 0, 'R$ ')); ?></div>
            <?php endif; ?>
            <?php if(($paymentItem->pay_installments_number ?? 1) > 1): ?>
                <div class="text-[9px] text-gray-400"><?php echo e($paymentItem->pay_installments_number); ?>x <?php echo e(toMoney($paymentItem->pay_installment_value ?? 0, 'R$ ')); ?></div>
            <?php endif; ?>
        <?php else: ?>
            <div class="text-xs text-gray-300">—</div>
        <?php endif; ?>
    ...
    <div class="flex items-center gap-2 border-l border-gray-200 pl-3 pr-1">
        <?php if(isAdmin()): ?>
            <button wire:click="abrirEditarPagamentoNoExibir('<?php echo e($paymentItem->id); ?>')"
                title="Editar"
            ...
            <button wire:click="$set('logTrasacao','<?php echo e($paymentItem->id); ?>')"
                title="Ver Log"
                class="w-7 h-7 flex items-center justify-center rounded-lg transition"
        ...
        <?php endif; ?>
        <button wire:click.prevent="paymentCheckProcessed('<?php echo e($paymentItem->id); ?>')"
...
<?php /**PATH /home/igrnc24/public_html_sistemas/campanhas-eventos-proeventpay-transicao/resources/views/livewire/dashboard/includes/payment-card.blade.php ENDPATH**/ ?>