<div class="sticky top-0 z-50" style="background: rgba(255,255,255,0.97); backdrop-filter: blur(14px); border-bottom: 1px solid #e5e7eb;">
    ...
    <div class="max-w-7xl mx-auto px-4 md:px-10 py-3 md:py-4">
        @if($isCampanhasPage || $isEventosPage)
            {{-- Mobile Layout --}}
            <div class="md:hidden space-y-3">
                {{-- Linha 1: Logo à esquerda + Minhas Compras à direita --}}
                <div class="flex items-center justify-between">
                    <a href="{{ route('home') }}">
                        <img src="{{ appLogo(true) }}" alt="{{ appName() }}" class="h-8">
                    </a>

                    {{-- Botão Minhas Doações/Compras Mobile --}}
                    @if(!auth()->user() || (!$isParticipantesPage && !$isCheckinPage))
                    @if(request()->routeIs('minhas-doacoes'))
                    <a
                        href="{{ route('campanhas-home') }}"
                    ...
                    @else
                    <a
                        href="{{ $isCampanhasPage ? route('minhas-doacoes') : route('minhas-compras') }}"
                    ...
                    >
                        ...
                        @endif
                        <span>{{ $isCampanhasPage ? 'Doações' : 'Compras' }}</span>
                    </a>
                    @endif
                    @endif
                </div>

                {{-- Linha 2: Parceiros 50% + Organizadores 50% --}}
                <div class="flex gap-2">
                    {{-- Dropdown Parceiros --}}
                    <div class="relative w-1/2" x-data="{ open: @entangle('showCustomerDropdown') }">
                        <button
                            @click="open = !open"
                        ...
                        >
                            <div class="flex flex-col items-start flex-1 min-w-0">
                                ...
                                <span class="text-xs font-semibold leading-snug truncate w-full uppercase" style="color: #374151;">
                                    @if($activeCustomerId)
                                        {{ Str::limit($customers->firstWhere('id', $activeCustomerId)->name_corporate ?? 'Todos', 12) }}
                                    @else
                                        Todos
                                    @endif
                                </span>
                        ...
                        >
                            <button
                                ...
                                class="w-full text-left px-4 py-2.5 text-xs transition-colors {{ !$activeCustomerId ? 'font-semibold' : '' }}"
                                style="color: {{ !$activeCustomerId ? '#1f2937' : '#6b7280' }};"
                            ...
                            @foreach($customers as $customer)
                                <button
                                    wire:click="$set('filterCustomer', '{{ $customer->id }}'); $set('filterCustomerSlug', '')"
                                    class="w-full text-left px-4 py-2.5 text-xs transition-colors {{ $activeCustomerId == $customer->id ? 'font-semibold' : '' }}"
                                    style="color: {{ $activeCustomerId == $customer->id ? '#1f2937' : '#6b7280' }};"
                                    onmouseover="this.style.background='#f3f4f6'" onmouseout="this.style.background=''"
                                >
                    ...
                    <div class="relative w-1/2" x-data="{ open: @entangle('showOrganizerDropdown') }">
                        ...
                        >
                            <button
                                ...
                                class="w-full text-left px-4 py-2.5 text-xs transition-colors {{ !$filterOrganizer ? 'font-semibold' : '' }}"
                                style="color: {{ !$filterOrganizer ? '#1f2937' : '#6b7280' }};"
                                onmouseover="this.style.background='#f3f4f6'" onmouseout="this.style.background=''"
                            >
                            ...
                            @foreach($organizers as $organizer)
                                <button
                                    ...
                                    class="w-full text-left px-3 py-2 transition-colors {{ $filterOrganizer == $organizer->id ? 'font-semibold' : '' }}"
                                    style="border-bottom: 1px solid #f3f4f6; color: {{ $filterOrganizer == $organizer->id ? '#1f2937' : '#6b7280' }};"
                                    onmouseover="this.style.background='#f3f4f6'" onmouseout="this.style.background=''"
                                >
                ...
                <div class="grid grid-cols-6 gap-2">
                    ...
                    {{-- Dropdown Status --}}
                    <div class="relative col-span-2" x-data="{ open: @entangle('showStatusDropdown') }">
                        <button
                            @click="open = !open"
                            class="w-full flex items-center justify-start text-left gap-2 px-3 py-2.5 rounded-lg transition-colors h-[42px]"
                            style="background: #f9fafb; border: 1px solid #e5e7eb;"
                        ...
                        >
                            <button
                                ...
                                class="w-full text-left px-4 py-2 text-xs transition-colors {{ $filterStatus === 'ativas' ? 'font-semibold' : '' }}"
                                style="color: {{ $filterStatus === 'ativas' ? '#1f2937' : '#6b7280' }};"
                                onmouseover="this.style.background='#f3f4f6'" onmouseout="this.style.background=''"
                            >
                                Ativas
                            </button>
                            <button
                                ...
                                class="w-full text-left px-4 py-2 text-xs transition-colors {{ $filterStatus === 'todas' ? 'font-semibold' : '' }}"
                                style="color: {{ $filterStatus === 'todas' ? '#1f2937' : '#6b7280' }};"
                                onmouseover="this.style.background='#f3f4f6'" onmouseout="this.style.background=''"
                            >
                                Todas
                            </button>
                            <button
                                ...
                                class="w-full text-left px-4 py-2 text-xs transition-colors {{ $filterStatus === 'finalizadas' ? 'font-semibold' : '' }}"
                                style="color: {{ $filterStatus === 'finalizadas' ? '#1f2937' : '#6b7280' }};"
                                onmouseover="this.style.background='#f3f4f6'" onmouseout="this.style.background=''"
                            >
                                Finalizadas
                            </button>
            ...
            <div class="hidden md:flex items-center gap-3">
                <!-- Logo -->
                <a href="{{ route('home') }}" class="shrink-0 flex items-center mr-2 md:mr-8">
                    <img src="{{ appLogo(true) }}" alt="{{ appName() }}" class="h-10 max-w-[180px] object-contain">
                </a>
                ...
                <div class="flex-1 min-w-[200px]">
                    <div class="relative">
                        ...
                        <input
                            type="text"
                            wire:model.debounce.300ms="search"
                            placeholder="{{ $isEventosPage ? 'Buscar eventos...' : 'Buscar campanhas...' }}"
                    ...
                    {{-- Dropdown Status --}}
                    <div class="relative" x-data="{ open: @entangle('showStatusDropdown') }">
                        <button
                            @click="open = !open"
                        ...
                        >
                            <button
                                wire:click="$set('filterStatus', 'ativas')"
                                class="w-full text-left px-4 py-2.5 text-sm transition-colors {{ $filterStatus === 'ativas' ? 'font-semibold' : '' }}"
                                style="color: {{ $filterStatus === 'ativas' ? '#1f2937' : '#6b7280' }};"
                                onmouseover="this.style.background='#f3f4f6'" onmouseout="this.style.background=''"
                            >
                                Ativas
                            ...
                            <button
                                wire:click="$set('filterStatus', 'todas')"
                                class="w-full text-left px-4 py-2.5 text-sm transition-colors {{ $filterStatus === 'todas' ? 'font-semibold' : '' }}"
                                style="color: {{ $filterStatus === 'todas' ? '#1f2937' : '#6b7280' }};"
                                onmouseover="this.style.background='#f3f4f6'" onmouseout="this.style.background=''"
                            >
                                Todas
                            ...
                            <button
                                wire:click="$set('filterStatus', 'finalizadas')"
                                class="w-full text-left px-4 py-2.5 text-sm transition-colors {{ $filterStatus === 'finalizadas' ? 'font-semibold' : '' }}"
                                style="color: {{ $filterStatus === 'finalizadas' ? '#1f2937' : '#6b7280' }};"
                                onmouseover="this.style.background='#f3f4f6'" onmouseout="this.style.background=''"
                            >
                                Finalizadas
                    ...
                    <div class="relative" x-data="{ open: @entangle('showCustomerDropdown') }">
                        ...
                        >
                            <div class="flex flex-col items-start flex-1 min-w-0">
                                ...
                                <span class="text-sm font-semibold leading-snug truncate w-full uppercase" style="color: #374151;">
                                    @if($activeCustomerId)
                                        {{ Str::limit($customers->firstWhere('id', $activeCustomerId)->name_corporate ?? 'Todos', 20) }}
                                    @else
                                        Todos
                                    @endif
                                </span>
                        ...
                        >
                            <button
                                wire:click="$set('filterCustomer', ''); $set('filterCustomerSlug', '')"
                                class="w-full text-left px-4 py-2.5 text-sm transition-colors {{ !$activeCustomerId ? 'font-semibold' : '' }}"
                                style="color: {{ !$activeCustomerId ? '#1f2937' : '#6b7280' }};"
                                onmouseover="this.style.background='#f3f4f6'" onmouseout="this.style.background=''"
                            >
                                Todos os Parceiros
                            ...
                            @foreach($customers as $customer)
                                <button
                                    wire:click="$set('filterCustomer', '{{ $customer->id }}'); $set('filterCustomerSlug', '')"
                                    class="w-full capitalize text-left px-4 py-2.5 text-sm transition-colors {{ $activeCustomerId == $customer->id ? 'font-semibold' : '' }}"
                                    style="color: {{ $activeCustomerId == $customer->id ? '#1f2937' : '#6b7280' }};"
                                    onmouseover="this.style.background='#f3f4f6'" onmouseout="this.style.background=''"
                                >
                                    {{ $customer->name_corporate }}
                    ...
                    {{-- Dropdown Organizador --}}
                    <div class="relative" x-data="{ open: @entangle('showOrganizerDropdown') }">
                        <button
                            @click="open = !open"
                            class="flex items-center justify-start text-left gap-2 pl-3 pr-2.5 py-1.5 rounded-lg transition-colors min-w-[160px]"
                            style="background: #f9fafb; border: 1px solid #e5e7eb;"
                        >
                            <div class="flex flex-col items-start flex-1 min-w-0">
                                ...
                                <span class="text-sm font-semibold leading-snug truncate w-full uppercase" style="color: #374151;">
                                    @if($filterOrganizer)
                                        {{ Str::limit($organizers->firstWhere('id', $filterOrganizer)->organizer_name ?? 'Todos', 20) }}
                                    @else
                                        Todos
                                    @endif
                                </span>
                        ...
                        >
                            <button
                                wire:click="$set('filterOrganizer', '')"
                                class="w-full text-left px-4 py-2.5 text-sm transition-colors {{ !$filterOrganizer ? 'font-semibold' : '' }}"
                                style="color: {{ !$filterOrganizer ? '#1f2937' : '#6b7280' }};"
                                onmouseover="this.style.background='#f3f4f6'" onmouseout="this.style.background=''"
                            >
                                Todos os Organizadores
                            ...
                            @foreach($organizers as $organizer)
                                <button
                                    wire:click="$set('filterOrganizer', '{{ $organizer->id }}')"
                                    title="{{ $organizer->organizer_name_full }}"
                                    class="w-full text-left px-4 py-2.5 transition-colors {{ $filterOrganizer == $organizer->id ? 'font-semibold' : '' }}"
                                    style="border-bottom: 1px solid #f3f4f6; color: {{ $filterOrganizer == $organizer->id ? '#1f2937' : '#6b7280' }};"
                                    onmouseover="this.style.background='#f3f4f6'" onmouseout="this.style.background=''"
                                >
                                    {{-- Linha 1: Cliente | Organization --}}
                                    <p class="text-[10px] uppercase truncate leading-tight" style="color: #9ca3af;">
                                        {{ Str::limit($organizer->customer->name_corporate ?? '', 22) }}
                                        @if($organizer->organization)
                                            <span class="mx-0.5">|</span>{{ Str::limit($organizer->organization->organization_name, 22) }}
                                        @endif
                                    </p>
                                    {{-- Linha 2: Setor --}}
                    ...
                    >
                        ...
                        @endif
                        <div>{{ $isCampanhasPage ? 'MINHAS DOAÇÕES' : 'MINHAS COMPRAS' }}</div>
                    </a>
                    @endif
                    @endif
            </div>
        @else
            {{-- Layout Normal (não campanhas) --}}
            <div class="flex justify-between items-center gap-4">
                <!-- Logo -->
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('home') }}">
                        {{-- White Label: Logo dinâmico --}}
                        <img src="{{ appLogo(true) }}" alt="{{ appName() }}" class="h-6 md:h-10">
                    </a>
                </div>
