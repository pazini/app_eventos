...
class SendCampaignPaymentApprovedEmail implements ShouldQueue
{
    ...
    public function handle()
    {
        try {
            ...
            $data = [
                ...
                'amount_paid' => number_format($this->order->amount_paid / 100, 2, ',', '.'),
                'paid_at' => $this->order->paid_at->format('d/m/Y H:i'),
                'payment_url' => $paymentUrl,
            ];

            $subject = $this->buildSubject($campaign->name ?? null);

            Mail::send('emails.campanha.pagamento-aprovado', $data, function ($message) use ($subject) {
                $message->to($this->order->buyer_email, $this->order->buyer_name)
                    ->subject($subject);
            });

            Log::info('[QUEUE] Email de pagamento aprovado enviado', [
            ...
            Log::error('[QUEUE] Erro ao enviar email de pagamento aprovado: ' . $e->getMessage(), [
    ...
    public function failed(\Throwable $exception)
    {
        ...
        Log::error('[QUEUE] Job de email de pagamento aprovado falhou após todas as tentativas', [
            'order_id' => $this->order->id ?? null,
    ...
    private function buildSubject(?string $campaignName): string
    {
        ...
        return mb_strtoupper('Pagamento Recebido, Obrigado - ' . $name . ' - ' . $this->order->order_control);
    }
    ...
    private function logNotification(string $status, ?string $subject, ?string $errorMessage = null, array $meta = []): void
    {
        try {
            ...
            Log::warning('[QUEUE] Falha ao registrar notificacao de pagamento aprovado', [
                'order_id' => $this->order->id ?? null,
                'error' => $e->getMessage(),
            ]);
        }
