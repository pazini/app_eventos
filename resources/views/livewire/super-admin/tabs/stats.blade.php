                        ...
                        <div class="ml-5 w-0 flex-1">
                            <dl>
                                ...
                                <dd class="flex items-baseline">
                                    ...
                                    @if(($stats['customers']['total'] ?? 0) > 0)
                                        <div class="ml-2 flex items-baseline text-sm text-green-600">
                                            {{ number_format($stats['customers']['active'] ?? 0) }} ativos
                                        </div>
                                    @endif
                                </dd>
                            </dl>
                        ...
                        <div class="ml-5 w-0 flex-1">
                            <dl>
                                ...
                                <dd class="flex items-baseline">
                                    ...
                                    @if(($stats['campaigns']['total'] ?? 0) > 0)
                                        <div class="ml-2 flex items-baseline text-sm text-green-600">
                                            {{ number_format($stats['campaigns']['active'] ?? 0) }} ativas
                                        </div>
                                    @endif
                                </dd>
                            </dl>
                            ...
                            <dl>
                                ...
                                <dd class="flex items-baseline">
                                    ...
                                    <div class="ml-2 flex items-baseline text-sm text-gray-500">
                                        / {{ number_format($stats['storage']['limit_mb'] ?? 0) }} MB
                                    </div>
                                </dd>
                            </dl>
                        </div>
                    </div>
                    {{-- Progress bar do storage --}}
                    @if(($stats['storage']['limit_mb'] ?? 0) > 0)
                        @php
                        ...
                        <div class="mt-3">
                            <div class="bg-gray-200 rounded-full h-2">
                                <div class="{{ $color_class }} h-2 rounded-full" style="width: {{ $usage_percentage }}%"></div>
                            </div>
                            <p class="text-xs text-gray-500 mt-1">{{ number_format($usage_percentage, 1) }}% usado</p>
                        </div>
                    @endif
                </div>
            </div>
                            ...
                            <dl>
                                ...
                                <dd class="text-lg font-semibold text-gray-900">
                                    @if($stats['last_activity'] ?? null)
                                        {{ \Carbon\Carbon::parse($stats['last_activity'])->diffForHumans() }}
                                    @else
                                        Nenhuma
                                    @endif
                                </dd>
            ...
            <div class="px-4 py-5 sm:p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        <dl class="space-y-3">
                            <div class="flex justify-between items-center py-2 border-b border-gray-100">
                                ...
                                <dd class="text-sm text-gray-900 font-mono">{{ Str::limit($app->id, 8) }}...</dd>
                            </div>
                            <div class="flex justify-between items-center py-2 border-b border-gray-100">
                                <dt class="text-sm font-medium text-gray-500">Criada em</dt>
                                <dd class="text-sm text-gray-900">{{ $app->created_at->format('d/m/Y H:i') }}</dd>
                            </div>
                            <div class="flex justify-between items-center py-2 border-b border-gray-100">
                                <dt class="text-sm font-medium text-gray-500">Última atualização</dt>
                                <dd class="text-sm text-gray-900">{{ $app->updated_at->format('d/m/Y H:i') }}</dd>
                            </div>
                            <div class="flex justify-between items-center py-2 border-b border-gray-100">
                                <dt class="text-sm font-medium text-gray-500">Branding atualizado</dt>
                                <dd class="text-sm text-gray-900">
                                    {{ $app->branding_updated_at ? $app->branding_updated_at->format('d/m/Y H:i') : 'Nunca' }}
                                </dd>
                            </div>
                            @if($app->app_limit_date)
                                <div class="flex justify-between items-center py-2 border-b border-gray-100">
                                    <dt class="text-sm font-medium text-gray-500">Expira em</dt>
                                    <dd class="text-sm {{ $app->app_limit_date->isPast() ? 'text-red-600' : 'text-gray-900' }}">
                                        {{ $app->app_limit_date->format('d/m/Y H:i') }}
                                        @if($app->app_limit_date->isPast())
                    ...
                    <div>
                        ...
                        <div class="space-y-2">
                            @foreach(['campaigns' => 'Campanhas', 'events' => 'Eventos', 'subscriptions' => 'Assinaturas', 'analytics' => 'Analytics', 'reports' => 'Relatórios', 'integrations' => 'Integrações'] as $key => $label)
